...
var App = function() {
	...
	var handleResponsive = function() {
		...
		$('.menu li').unbind().click(function(e)
		{
			// e.preventDefault();
			// var check = $(this).children('.sub-menu').css('display');
			// alert(check);
			$('.sub-menu').fadeOut(500);
			$('.menu').find('img').removeClass('transform-180');
			if($(this).children('.sub-menu').css('display') == 'none')
			{
				$(this).children('.sub-menu').fadeIn(500);
				$(this).find('img').addClass('transform-180');
				// return true;
			}
			else
			{
				$(this).children('.sub-menu').fadeOut(500);
				$(this).find('img').removeClass('transform-180');
				// return true;
			}
