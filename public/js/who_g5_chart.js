$(document).ready(function() {
    ...
    if (gender == 'male') {
        generateWhoChart('wt-chart-container', 'wt_provider', 'Weight (kg)', 13, 45, 'wt_value_count', 'Weight', 30, 25, 20, 60, 123);
        generateWhoChart('ht-chart-container', 'ht_provider', 'Height (cm)', 100, 195, 'ht_value_count', 'Height', 12, 9, 7, 60, 235);
        generateWhoChart('bmi-chart-container', 'bmi_provider', 'BMI (kg/m2)', 12, 30, 'bmi_value_count', 'BMI', 12, 9, 7, 60, 235);
    } else {
        generateWhoChart('wt-chart-container', 'wt_provider', 'Weight (kg)', 13, 47, 'wt_value_count', 'Weight', 30, 25, 20, 60, 123);
        generateWhoChart('ht-chart-container', 'ht_provider', 'Height (cm)', 95, 180, 'ht_value_count', 'Height', 12, 9, 7, 60, 235);
        generateWhoChart('bmi-chart-container', 'bmi_provider', 'BMI (kg/m2)', 12, 29.5, 'bmi_value_count', 'BMI', 12, 9, 7, 60, 235);
    }

    function generateWhoChart(containter_name, provider_name, chart_name, min_y_value, max_y_value, value_count, chart_title, x_axis_lg, x_axis_md, x_axis_sm, x_axis_starting_value, x_axis_ending_value) {
        ...
        var xAxis1 = chart.xAxes.push(am5xy.ValueAxis.new(root, {
            min: x_axis_starting_value,
            max: x_axis_ending_value,
            strictMinMax: true,
            renderer: xRenderer1,
            tooltip: false
        }));
        ...
        for (var providerName in data) {
            ...
            for (var itemName in providerData) {
                if (itemName != "quantity") {
                    ...
                    if ((temp_count == 59 && chart_title == 'Weight') || (temp_count == 166 && chart_title == 'Height') || (temp_count == 166 && chart_title == 'BMI')) {
                        show_bullets = true;
                    } else {
                        show_bullets = false;                        
                    }
                    var item_name = itemName;
                    if (((temp_count == 61 || temp_count == 62) || item_name == 0 && chart_title == 'Weight') || ((temp_count > 170) || item_name == 0 && chart_title != 'Weight')) {
                        item_name = '';
                    }
        ...
        $.each(chartData, function(key, value) {
            if (chart_title == 'Weight') {
                ...
                var current_grid = key % 3;
                var stroke_color = '#D3D3D3';
                if (current_grid == 0 && key < 170 && key % 12 != 0) {
                    var range = xAxis.makeDataItem({});
                    xAxis.createAxisRange(range);
                    range.set("category", chartData[key].category);
                    var grid = range.get("grid");
        ...
        if (chart_title == 'Height') {
            for (i = min_y_value; i < max_y_value; i++) {
                var grid_value = i + 1;
                if (grid_value % 10 != 0 && grid_value % 5 == 0) {
                    var range = yAxis.makeDataItem({
                        value: grid_value,
                    });
                    yAxis.createAxisRange(range);
        ...
        if (chart_title == 'BMI') {
            for (i = min_y_value; i < max_y_value; ) {
