 ...
 * @author Manikandan M
 */
class ProblemPostnatalList extends Model
{

  /**
    *@var $table string
    */
   protected $table = 'pb_postnatal_list';

   /**
    *@var $primaryKey string  
    */
   protected $primaryKey = 'pb_postnatal_id';
    ...
    */
    ...
    public function GetList($page=1, $limit=50, $condition = array(), $order=array(), $slug = '', $status = '') 
    {
    	...
    	$results = \DB::table('baby')
    	            ->select('baby.BabyId', 'baby.BabyName', 'baby.DOB', 'baby.BabyBloodGroup', 'baby.BMrNo')
                    // ->leftjoin('postnatal_discharge', 'postnatal_discharge.BabyId', '=', 'baby.BabyId')
                    ->whereIn('baby.BabyId', function ($query) {
    	            	$query->select('BabyId')
                        ->from('postnatal_admission')
                        ->groupby('BabyId')
                        ->get()
                        ->toArray();
    	            })
                    ->whereIn('baby.BabyId', function ($query) use ($search_txt, $status) {
                        $query->select('BabyId')
                ...
                elseif ($status == 'discharged') {
                    ...
                    ->where(function ($query) use ($search_txt, $checkdate) {
                        if ($search_txt) {
                            $query->where('BabyName', 'ilike', '%'.$search_txt.'%');
                            $query->orwhere('baby.BMrNo', 'ilike', '%'.$search_txt.'%');
                            $query->orwhere('BabyBloodGroup', 'ilike', '%'.$search_txt.'%');
                            // if (empty($search_txt)) {
                            //     $query->whereIn('discharge_status',['Inpatient']);
                            // }
                        } else if ($checkdate) {
     ...
     /**
     *This method to fetch the problem base postnatal baby list count
     *
     * @return integer  
     */
    ...
    /**
     *This method to fetch the problem base postnatal baby admission list
     *
    ...
    public function GetAdmissionList($baby_id) 
    {
    	$results = \DB::table('baby_admission')
                    ->select('baby_admission.AdmissionId', 'baby_admission.BabyId', 'baby_admission.episodes')
    ...
    /**
     *This method to fetch the problem base postnatal baby admission episodes list
     *
     * @param $baby_id 
     * 
     * @param $admission_id
    ...
    {
    	$result = \DB::table('pb_postnatal_list')
    	            ->select('pb_postnatal_list.*', 'baby.*', 'pd_published.*')
                    ->join('pd_published', 'pd_published.problem_id', '=', 'pb_postnatal_list.problem_id')
    	            ->join('baby', 'baby.BabyId', '=', 'pb_postnatal_list.baby_id')
    	            ->where(['pb_postnatal_list.baby_id'=>$baby_id, 'pb_postnatal_list.admission_id'=>$admission_id])
                    ->where('pb_postnatal_list.IsDeleted', 0)
                    ->get();
        // $results = $results->unique('problem_id')->values()->all();
        $result = \SiteHelpers::convert_obj_to_array($result->toArray());
        $result = \SiteHelpers::unique_multidim_array($result, 'problem_id');
        $result = \SiteHelpers::convert_array_to_object($result);  
        // $results = $result->values()->all();
        return $result;           
    ...
    {
        $results =  \DB::table('pb_postnatal_list')
                         ->select('pb_postnatal_id')
                         ->where(['baby_id'=>$baby_id, 'admission_id'=>$admissionId, 'problem_id'=>$problem_id])
                         ->where('IsDeleted', 0)
                         ->first();
       return $results;
    ...
    public function GetEpisodes() 
    {
		...
		return $this->hasMany('App\Models\ProblemPostnatalEpisode', 'pb_postnatal_id', 'pb_postnatal_id')
        ->where('IsDeleted', '0')
    ...
    public function GetProblemList($baby_id, $admissionid)
    {
         $results = \DB::table('pd_published')
                     ->select('problem_name', 'problem_id', 'DateModified')
