...
class ReferralMaster extends Model
{
    ...
    *@var $fillable
	*/
	protected $fillable = ['doctor_name', 'hospital_name', 'hospital_number', 'hospital_email', 'status', 'user_added', 'user_modified', 'date_added', 'date_modified',
	...
	public static function  list($page = 1, $limit = 50, $condition, $order = array(), $slug = '')
	{
		...
		$results    = DB::table('mas_referral')
		->select('*')
		->where('is_deleted', 0)
		->where(function ($query) use ($search_txt) {
			if ($search_txt) {
				$query->orWhere('doctor_name', 'ilike', '%'.$search_txt.'%');
				$query->orWhere('hospital_name', 'ilike', '%'.$search_txt.'%');
			}
		});
