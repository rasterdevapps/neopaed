...
class ManualLogDetails extends Model
{
   protected $table = 'manual_log_dtl';
	 protected $primaryKey = 'id';
	 public    $timestamps  =  false;
	 protected $fillable = ['log_hdr_id', 'manual_intf_data_id', 'loinc_local_map_id', 'loinc_code','loinc_version','intf_ref_name', 'intf_ref_value', 'component', 'property', 'time', 'system', 'scale', 'method', 'classtype', 'active_flag', 'create_user_id', 'create_tstamp', 'modify_user_id', 'modify_tstamp', 'device_parameter_map_id', 'observation_type_id', 'manual_vendor_ans_list_map_id', 'intf_ref_code', 'is_calulated'];
    ...
    public static function GetToggleValues($nurse_id, $local_code)
    {
        return \DB::table('nurse_main_sheet')
                    ->selectRaw('manual_log_hdr.id as log_hdr, manual_log_dtl.id as log_dtl_id, manual_log_hdr.day_id, local_code_group.local_code')
    ...
    {
        return \DB::table('manual_log_dtl')
                   ->select('manual_log_dtl.*', 'local_code_group.*','manual_log_dtl.id as log_id')
                   ->join('local_code_group', 'local_code_group.id','=','manual_log_dtl.loinc_local_map_id')
                   ->where('manual_log_dtl.log_hdr_id', $header_id)
                   ->whereIn('local_code_group.local_code', $local_code)
                   ->get();
     ...
     * @return array of object 
     */
    public static function Getpreviousbasic($local_code , $baby_id)
    {
        return \DB::table('manual_log_dtl')
                   ->select('manual_log_dtl.*', 'local_code_group.*','manual_log_dtl.id as log_id')
                   ->join('local_code_group', 'local_code_group.id','=','manual_log_dtl.loinc_local_map_id')
    ...
    public static function GetprescriptionDetails($baby_id,  $sheet_date, $admission_id)
    {
        ...
        return \DB::table('manual_log_hdr')
                   ...
                   ->join('local_code_group', 'local_code_group.id', '=','manual_log_dtl.loinc_local_map_id')
                   ->where('manual_log_hdr.baby_id', $baby_id)
                   ->where('manual_log_hdr.visit_date', date('Y-m-d', strtotime($sheet_date)))
    ...
    public static function getremovedetails($day_id, $log_ids, $local_codes)
    {
          ...
          return \DB::table('manual_log_dtl')
                      ...
                      ->where('manual_log_hdr.day_id', $day_id)
                      ->whereNotIn('manual_log_dtl.id',$log_ids)
                      ->delete();
    ...
    public static function getgraphicaldata($baby_id, $periods_start, $periods_end, $vendilator_parameters = array()) 
    {
        ...
        $results = \DB::table('nurse_main_sheet')
                   ->select('manual_log_hdr.sender_time', 'local_code_group.local_code', 'manual_log_dtl.intf_ref_value')
                   ->join('manual_log_hdr', 'manual_log_hdr.day_id','=','nurse_main_sheet.id')
                   ->join('manual_log_dtl', 'manual_log_dtl.log_hdr_id','=','manual_log_hdr.id')
                   ->join('local_code_group','local_code_group.id','=','manual_log_dtl.loinc_local_map_id')
        ...
        $results = $results->where('nurse_main_sheet.baby_id', '=',$baby_id)
                           ->orderBy('manual_log_hdr.sender_time','asc')
                           ->get();
    ...
    public static function getDrugByDate($from_time, $to_time) {
       return \DB::table('manual_log_dtl')
                  ->select('manual_log_dtl.*', 'local_code_group.*','manual_log_dtl.id as log_id', 'manual_log_hdr.sender_time')
                  ->join('local_code_group', 'local_code_group.id','=','manual_log_dtl.loinc_local_map_id')
                  ->join('manual_log_hdr', 'manual_log_hdr.id','=','manual_log_dtl.log_hdr_id')
                  // ->where('manual_log_dtl.log_hdr_id', $previous_day->id)
                  ->whereIn('local_code_group.local_code', DialpadSupportProperty::DRUG_FLUIDS)   
                  ->where('manual_log_hdr.sender_time', '>=' ,date('Y-m-d H:i',strtotime($from_time)))
                  ->where('manual_log_hdr.sender_time', '<=' ,date('Y-m-d H:i',strtotime($to_time)))
                  ->get();
    }
    ...
    {
       $results = \DB::table('manual_log_dtl')
                      ->select('id')
    ...
    public static function emrLogIdDetail($start_date, $end_date, $baby_id)
    {
       $results = \DB::table('manual_log_dtl')
                      ...
                      ->whereBetween('manual_log_dtl.create_tstamp',[$start_date, $end_date])
                      // ->where('manual_log_dtl.create_tstamp', '>=', $start_date)
                      // ->where('manual_log_dtl.create_tstamp', '<', $end_date)
                      ->where('manual_log_dtl.intf_ref_value','<>',0)
                      ->get();
       return $results;
    }
    ...
    public static function GetFhirPrescribeddata($local_code, $baby_id, $admission_id, $today_start, $today_current)
    {
        return \DB::table('manual_log_dtl')
                   ...
                   ->where('manual_log_hdr.baby_id', $baby_id)
                   ->where('manual_log_hdr.admission_id', $admission_id)
                   ->where('manual_log_hdr.sender_time', '>=', $today_start)
                   ->where('manual_log_hdr.sender_time', '<=', $today_current)
                   ->orderBy('manual_log_dtl.time', 'desc')
                   ->get()->toArray();
    ...
    public static function compareFhirPrescribeddata($local_code, $baby_id, $admission_id, $today_start, $today_current, $values)
    {
        return \DB::table('manual_log_dtl')
                   ...
                   ->join('local_code_group', 'local_code_group.id','=','manual_log_dtl.loinc_local_map_id')
                   ->whereIn('local_code_group.local_code', $local_code)
                   ->whereIn('manual_log_dtl.intf_ref_value', $values)
                   ->where('manual_log_hdr.baby_id', $baby_id)
                   ->where('manual_log_hdr.admission_id', $admission_id)
                   ->where('manual_log_hdr.sender_time', '>=', $today_start)
                   ->where('manual_log_hdr.sender_time', '<=', $today_current)
    ...
    {
       return \DB::table('manual_log_hdr')
                  ->select('manual_log_dtl.*', 'manual_log_hdr.*', 'manual_log_dtl.id as log_id')
                  ->join('manual_log_dtl', 'manual_log_dtl.log_hdr_id', '=', 'manual_log_hdr.id')
                  ->where('manual_log_dtl.loinc_code', '=', $loinc_code)
    ...
    public static function get_lab_values($admission_id, $lab_codes) {
      $result = \DB::table('manual_log_dtl')
                ->select('manual_log_dtl.*', 'local_code_group.local_code', 'manual_log_hdr.sender_time')
                ->join('manual_log_hdr',  'manual_log_hdr.id', '=', 'manual_log_dtl.log_hdr_id')
                ->join('local_code_group',  'local_code_group.id', '=', 'manual_log_dtl.loinc_local_map_id')
                ->where('manual_log_hdr.admission_id', $admission_id)
                ->whereIn('local_code_group.local_code', $lab_codes)
                ->get();
      return $result;
