...
class EmrMoniterValues extends Model
{
   protected $table = 'emr_moniter_values';
    ...
    {
        return \DB::table('emr_moniter_values')
                   ->where('id', $log_id)
    ...
    public static function GetToggleValues($nurse_id, $local_code)
    {
        return \DB::table('nurse_main_sheet')
                    ->selectRaw('emr_log_hdr.id as log_hdr, emr_moniter_values.id as log_dtl_id, emr_log_hdr.day_id, local_code_group.local_code')
                    ->join('emr_log_hdr', 'emr_log_hdr.day_id','=','nurse_main_sheet.id')
                    ->join('emr_moniter_values','emr_moniter_values.log_hdr_id', '=','emr_log_hdr.id')
    ...
    {
        return \DB::table('emr_moniter_values')
                   ->select('emr_moniter_values.*', 'local_code_group.*','emr_moniter_values.id as log_id')
                   ->join('local_code_group', 'local_code_group.id','=','emr_moniter_values.loinc_local_map_id')
                   ->where('emr_moniter_values.log_hdr_id', $header_id)
                   ->whereIn('local_code_group.local_code', $local_code)
                   ->get();
    ...
    {
        return \DB::table('emr_moniter_values')
                   ->select('emr_moniter_values.*','local_code_group.local_code')
                   ->join('local_code_group', 'local_code_group.id','=','emr_moniter_values.loinc_local_map_id')
    ...
    public static function getremovedetails($day_id, $log_ids, $local_codes)
    {
          ...
          return \DB::table('emr_moniter_values')
                      ...
                      ->where('emr_log_hdr.day_id', $day_id)
                      ->whereNotIn('emr_moniter_values.id',$log_ids)
                      ->delete();
    ...
    public static function getgraphicaldata($baby_id, $periods_start, $periods_end, $vendilator_parameters = array()) 
    {
        ...
        $results = \DB::table('nurse_main_sheet')
                   ->select('emr_log_hdr.sender_time', 'local_code_group.local_code', 'emr_moniter_values.intf_ref_value')
                   ->join('emr_log_hdr', 'emr_log_hdr.day_id','=','nurse_main_sheet.id')
                   ->join('emr_moniter_values', 'emr_moniter_values.log_hdr_id','=','emr_log_hdr.id')
                   ->join('local_code_group','local_code_group.id','=','emr_moniter_values.loinc_local_map_id')
    ...
    public static function getDrugByDate($from_time, $to_time) {
       return \DB::table('emr_moniter_values')
                  ->select('emr_moniter_values.*', 'local_code_group.*','emr_moniter_values.id as log_id', 'emr_log_hdr.sender_time')
                  ->join('local_code_group', 'local_code_group.id','=','emr_moniter_values.loinc_local_map_id')
                  ->join('emr_log_hdr', 'emr_log_hdr.id','=','emr_moniter_values.log_hdr_id')
                  // ->where('emr_moniter_values.log_hdr_id', $previous_day->id)
    ...
    {
       $results = \DB::table('emr_moniter_values')
                      ->select('emr_moniter_values.*')
                      ->join('emr_log_hdr', 'emr_log_hdr.id','=','emr_moniter_values.log_hdr_id')
                      ->where('emr_log_hdr.baby_id', $baby_id)
                      // ->where('log_hdr_id',$refId)
                      ->whereBetween('emr_moniter_values.create_tstamp',[$start_date, $end_date])
                      // ->where('emr_moniter_values.create_tstamp', '>=', $start_date)
                      // ->where('emr_moniter_values.create_tstamp', '<', $end_date)
                      ->where('emr_moniter_values.intf_ref_value','<>',0)
                      ->get();
       return $results;
    }
    ...
    public static function GetFhirPrescribeddata($local_code, $baby_id, $admission_id, $today_start, $today_current)
    {
        return \DB::table('emr_moniter_values')
                   ...
                   ->orderBy('emr_moniter_values.time', 'desc')
                   ->get()->toArray();
    ...
    public static function compareFhirPrescribeddata($local_code, $baby_id, $admission_id, $today_start, $today_current, $values)
    {
        return \DB::table('emr_moniter_values')
                   ...
                   ->join('local_code_group', 'local_code_group.id','=','emr_moniter_values.loinc_local_map_id')
                   ->whereIn('local_code_group.local_code', $local_code)
                   ->whereIn('emr_moniter_values.intf_ref_value', $values)
                   ->where('emr_log_hdr.baby_id', $baby_id)
                   ->where('emr_log_hdr.admission_id', $admission_id)
                   ->where('emr_log_hdr.sender_time', '>=', $today_start)
                   ->where('emr_log_hdr.sender_time', '<=', $today_current)
    ...
    {
       return \DB::table('emr_log_hdr')
                  ->select('emr_moniter_values.*', 'emr_log_hdr.*', 'emr_moniter_values.id as log_id')
                  ->join('emr_moniter_values', 'emr_moniter_values.log_hdr_id', '=', 'emr_log_hdr.id')
                  ->where('emr_moniter_values.loinc_code', '=', $loinc_code)
    ...
    public static function getHeroScore($search = '', $page = 1, $limit = 50, $order = array(), $slug = '')
    {
      $limitstart    = (empty($page) || $page == 1) ? 0 : (($page-1)*$limit);
      $limitend      = $limit;

      $mrn = isset($search['mrn']) && !empty($search['mrn']) ? $search['mrn'] : '';
      $from_date = isset($search['fromdate']) && !empty($search['fromdate']) ? date('Y-m-d', strtotime($search['fromdate'])) . ' 00:00' : '';
      $to_date = isset($search['todate']) && !empty($search['todate']) ? date('Y-m-d', strtotime($search['todate'])) . ' 24:00' : '';


      $results_values = \DB::table('baby')
                  ->select('result_date_time', 'mean', 'BabyName', 'BMrNo', 'BabyId')
                  ->leftjoin('emr_log_hdr', 'emr_log_hdr.baby_id', '=', 'baby.BabyId')
                  ->leftjoin('emr_moniter_values', 'emr_moniter_values.log_hdr_id', '=', 'emr_log_hdr.id')
                  ->where('loinc_local_map_id', 9)
                  ->where(function ($query) use ($mrn, $from_date, $to_date) {
                    if (!empty($mrn)) {
                      $query->whereRaw('trim("BMrNo") like '."'%".trim($mrn)."%'");
                    } 
                  });

      $results_values_discharged = \DB::table('baby')
                  ->select('result_date_time', 'mean', 'BabyName', 'BMrNo', 'BabyId')
                  ->leftjoin('emr_log_hdr', 'emr_log_hdr.baby_id', '=', 'baby.BabyId')
                  ->leftjoin('emr_moniter_values_discharged', 'emr_moniter_values_discharged.log_hdr_id', '=', 'emr_log_hdr.id')
                  ->where('loinc_local_map_id', 9)
                  ->where(function ($query) use ($mrn, $from_date, $to_date) {
                    if (!empty($mrn)) {
                      $query->whereRaw('trim("BMrNo") like '."'%".trim($mrn)."%'");
        ...
        $results = $results_values_discharged->union($results_values);
        ...
        if ($slug == 1)  {
          $result['result'] = $results->get();
            ...
            $result['min'] = date('Y-m-d', strtotime($results->get()->min('result_date_time'))).' 00:00:00';
            $result['max'] = $results->get()->max('result_date_time');
  ...
  public static function getTotalHeroScore()
  {

    $result = \DB::table('baby')
                  ->leftjoin('emr_log_hdr', 'emr_log_hdr.baby_id', '=', 'baby.BabyId')
                  ->leftjoin('emr_moniter_values', 'emr_moniter_values.log_hdr_id', '=', 'emr_log_hdr.id')
                  ->where('loinc_local_map_id', 35)
                  ->groupBy('loinc_local_map_id')
                  ->count();
  ...
  public static function getInterfacingData($baby_id, $admission_id, $nurse_sheet_id, $start_time = '', $end_time = '') {
        ...
        $monitor_results = \DB::table('emr_moniter_values')
                            ->select('emr_moniter_values.intf_ref_value', 'local_code_group.local_code')
                            ->join('emr_log_hdr', 'emr_log_hdr.id', '=', 'emr_moniter_values.log_hdr_id')
                            ->join('local_code_group', 'local_code_group.id', '=', 'emr_moniter_values.loinc_local_map_id')
        ...
        $monitor_results =  $monitor_results->whereBetween('result_date_time', [$start_time, $end_time]);
        }
        $monitor_results = $monitor_results->where('emr_moniter_values.create_user_id', 2)
    ...
    public static function getHeroScoreForExport($baby_mrn, $baby_name, $baby_id, $min, $max) {
        $results = \DB::select('WITH hours as (Select * from generate_series(\''.$min.'\'::timestamp, \''.$max.'\'::timestamp, INTERVAL \'1 hour\') as hr)
SELECT hours.hr, coalesce(hero.mean, \'\') as mean, hr::date as date, hr::time as time FROM hours 
left join(
    select distinct on ("emr_moniter_values".result_date_time) result_date_time, mean from emr_moniter_values 
join emr_log_hdr on "emr_moniter_values".log_hdr_id = emr_log_hdr.id
where "emr_moniter_values".loinc_local_map_id = 9 and emr_log_hdr.baby_id = '.$baby_id.' union
select distinct on ("emr_moniter_values_discharged".result_date_time) result_date_time, mean from emr_moniter_values_discharged 
join emr_log_hdr on "emr_moniter_values_discharged".log_hdr_id = emr_log_hdr.id
where "emr_moniter_values_discharged".loinc_local_map_id = 9 and emr_log_hdr.baby_id = '.$baby_id.'
  ) hero on hero.result_date_time = hours.hr
order by hr asc');
        return $results;
    }
