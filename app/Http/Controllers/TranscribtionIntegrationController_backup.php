...
class TranscribtionIntegrationController extends Controller
{
    ...
    public function createUpdateNeonatalProforma($input, $baby_data)
    {
        ...
        $proforma['TimeOf1stGasp'] = isset($input['timeOf1stGasp']) && !empty($input['timeOf1stGasp']) ? $input['timeOf1stGasp'] : null;
        $proforma['timeofgasp_status'] = isset($input['timeOf1stGaspInMinutes']) && !empty($input['timeOf1stGaspInMinutes']) ? $input['timeOf1stGaspInMinutes'] : null;
        $proforma['RegularRespiration'] = isset($input['regularRespiration']) && !empty($input['regularRespiration']) ? $input['regularRespiration'] : null;
        $proforma['regularrespiration_status'] = isset($input['regularRespirationMinutes']) && !empty($input['regularRespirationMinutes']) ? $input['regularRespirationMinutes'] : null;
        ...
        $proforma['bag_mask_ventilator_duration'] = isset($input['bagMaskVentilationDuration']) && !empty($input['bagMaskVentilationDuration']) ? $input['bagMaskVentilationDuration'] : null;
        $proforma['bag_mask_ventilator_min'] = isset($input['bagMaskVentilationDurationMin']) && !empty($input['bagMaskVentilationDurationMin']) ? $input['bagMaskVentilationDurationMin'] : null;
    ...
    public function createUpdateOpNeonatal($input, $baby_data)
    {
        ...
        $op['neurosonogram'] = ($medical_history['neurosonogram'] ?? false) ? 2 : 1;
        $op['echocardiogram'] = ($medical_history['echocardiogram'] ?? false) ? 2 : 1;
        $op['fee_status'] = ($follow_up['fee']['status'] ?? false) ? 1 : 0;
                ...
                if (isset($visit_dtl->AdmissionId)) {
                    ...
                    if (isset($nicu_dtl->NicuId)) {
                        ...
                        if (isset($discharge_dtl['date']) && strtotime($discharge_dtl['date'])) {
                            $discharge_time = strtotime($discharge_dtl['date']);
                            $nicu['DischargeDate'] = date('Y-m-d', $discharge_time);
                            $nicu['DischargeTransferedTime'] = date('h', $discharge_time);
                            $nicu['DischargeTransferedTime_MINS'] = date('i', $discharge_time);
                            $nicu['DischargeTransferedTime_AM'] = date('A', $discharge_time);
                        }
                        if (isset($discharge_dtl['correctedGestation'])) {
                            $nicu['dcg_weeks'] = $discharge_dtl['correctedGestation']['weeks'] ?? null;
                            $nicu['dcg_days'] = $discharge_dtl['correctedGestation']['days'] ?? null;
                            $nicu['corrected_gestation'] = json_encode($discharge_dtl['correctedGestation']);
                        }
                        ...
                        if ($ip_number_old) {
                            ...
                            IpNumber::where('id', $ip_number_old->id)->update($ip_data);
                        } else {
