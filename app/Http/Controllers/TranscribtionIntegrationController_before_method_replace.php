...
class TranscribtionIntegrationController extends Controller
{
    ...
    private function mapNeonatalInput($input)
    {
        ...
        $mapped['dateTime'] = $birth_source['dateTime'] ?? ($input['dateTime'] ?? (($input['dob'] ?? null) ? $input['dob'] . ' ' . date('H:i:s') : date('Y-m-d H:i:s')));
    ...
    public function createUpdateNeonatalProforma($input, $baby_data)
    {
        ...
        if (isset($input['medicalProblem'])) {
            ...
            for ($i = 0; $i < $pbm_length; $i++) {
                if ($input['medicalProblem'][$i] != '' && $input['medicalProblem'][$i] != 0) {
                    $pbms = array(
                        'Problem' => $input['medicalProblem'][$i]["problem"],
                        'Medication' => $input['medicalProblem'][$i]["medication"],
        ...
        if (isset($input['liveBirthBabyDetails'])) {
            ...
            for ($i = 0; $i < $delivery_length; $i++) {
                $deli_data = array(
                    'Year' => $input['liveBirthBabyDetails'][$i]["birthYear"],
                    'Place' => $input['liveBirthBabyDetails'][$i]["place"],
                    'Delivery' => $input['liveBirthBabyDetails'][$i]["typeOfDelivery"],
                    'Complications' => $input['liveBirthBabyDetails'][$i]["complications"],
                    'Gender' => $input['liveBirthBabyDetails'][$i]["gender"],
                    'GA' => $input['liveBirthBabyDetails'][$i]["gestation"],
                    'BW' => $input['liveBirthBabyDetails'][$i]["birthWeight"],
                    'Health' => $input['liveBirthBabyDetails'][$i]["health"],
                    'MotherId' => $baby_data->MotherId,
                    'details' => $input['liveBirthBabyDetails'][$i]["details"]
                );
        ...
        $proforma['TestDate'] = date('Y-m-d', strtotime($input['dateTime']));
        ...
        if (isset($input['pregnancyComplicationsDetails'])) {
            $com_length = sizeof($input['pregnancyComplicationsDetails']);
            for ($i = 0; $i < $com_length; $i++) {
                $input['pregnancyComplicationsDetails'][$i]["duration"] = empty($input['pregnancyComplicationsDetails'][$i]["duration"]) ? null : $input['pregnancyComplicationsDetails'][$i]["duration"];
                $input['pregnancyComplicationsDetails'][$i]["durationType"] = empty($input['pregnancyComplicationsDetails'][$i]["durationType"]) ? null : $input['pregnancyComplicationsDetails'][$i]["durationType"];

                if ($input['pregnancyComplicationsDetails'][$i]['complication'] != '' && $input['pregnancyComplicationsDetails'][$i]['complication'] != 0) {
                    $comps = array(
                        'Complication' => $input['pregnancyComplicationsDetails'][$i]['complication'],
                        'Treatment' => $input['pregnancyComplicationsDetails'][$i]['treatment'],
                        'duration_in_weeks' => $input['pregnancyComplicationsDetails'][$i]['duration'],
                        'duration_unit' => $input['pregnancyComplicationsDetails'][$i]['durationType'],
                        'AdmissionId' => 1,
                        'BabyId' => $baby_data->BabyId,
        ...
        if (isset($input['datingScanDetails']) && !empty($input['datingScanDetails'])) {
            ...
            $usg_parameters['Gestation'] = $input['datingScanDetails']['gestation'];
            $usg_parameters['Finding'] = $input['datingScanDetails']['findings'];
        ...
        if (isset($input['anomalyScanDetails']) && !empty($input['anomalyScanDetails'])) {
            ...
            $usg_parameters['Gestation'] = $input['anomalyScanDetails']['gestation'];
            $usg_parameters['Finding'] = $input['anomalyScanDetails']['findings'];
            Usg::create($usg_parameters);
        }
        unset($usg_parameters);
        ...
        if (isset($input['otherScanDetails']) && !empty($input['otherScanDetails'])) {

            for ($i = 0; $i < count($input['otherScanDetails']); $i++) {
                $usg = array(
                    'date' => isset($input['otherScanDetails'][$i]["date"]) && !empty($input['otherScanDetails'][$i]["date"]) ? date('Y-m-d', strtotime($input['otherScanDetails'][$i]["date"])) : null,
                    'Gestation' => $input['otherScanDetails'][$i]["gestation"],
                    'Finding' => $input['otherScanDetails'][$i]["findings"],
                    'MotherId' => $baby_data->MotherId,
                    'BabyId' => $baby_data->BabyId,
                    'flags' => $this->usg_flags,
                    'type' => 3,
        ...
        if (isset($input['dopplerScanDetails']) && !empty($input['dopplerScanDetails'])) {

            for ($i = 0; $i < count($input['dopplerScanDetails']); $i++) {
                $usg = array(
                    'date' => isset($input['dopplerScanDetails'][$i]["date"]) && !empty($input['dopplerScanDetails'][$i]["date"]) ? date('Y-m-d', strtotime($input['dopplerScanDetails'][$i]["date"])) : null,
                    'Gestation' => $input['dopplerScanDetails'][$i]["gestation"],
                    'Finding' => $input['dopplerScanDetails'][$i]["findings"],
                    'MotherId' => $baby_data->MotherId,
                    'BabyId' => $baby_data->BabyId,
                    'flags' => $this->usg_flags,
                    'type' => 4,
...
//        }
    ...
    public function createUpdateBabyDetails($data)
    {
        ...
        if (count($babyIsExist) > 0) {
            ...
            $mother['MotherName'] = 'M/O' . $mother_data['name']['first'];
            ...
            if (isset($patient_response['data']) && !empty($patient_response['data'])) {
                ...
                $baby['Sex'] = ucfirst(strtolower($baby_data['sex']));
    ...
    public function createUpdateOpNeonatal($input, $baby_data)
    {
        ...
        $baby['Sex'] = $baby_details['sex'] ?? null;
        $baby['BirthStatus'] = $baby_details['birthStatus'] ?? null;
        $baby['BirthOrder'] = $baby_details['birthOrder'] ?? null;
        $baby['BabyBloodGroup'] = $baby_details['bloodGroup'] ?? null;
        $baby['BirthWeight'] = $baby_details['birthWeight'] ?? null;
        $gestation = $baby_details['gestation'] ?? null;
        $baby['Gestation'] = json_encode($gestation);
        $baby['g_weeks'] = $gestation['weeks'] ?? null;
        $baby['g_days'] = $gestation['days'] ?? null;
