...
class NutritionChartController extends Controller
{
    ...
    {
        return view('reports.nutrition_chart.list');
    }
    ...
    public function getNutritionData(Request $request)
    {
        ...
        return view('reports.nutrition_chart.list', compact('drug_infused', 'day_wise_milk_volume', 'mrn', 'sheet_date_list'));
    ...
    function processData($mrn) {
        ...
        if ($mrn > 0) {
            ...
            $baby_result = Baby::select('BabyId')->where('BMrNo', $mrn)->where('IsDeleted', '0')->first();
            ...
            foreach ($sheet_date_list as $sheet_date) {
                ...
                ->map(function ($prescription_data, $prescription_id) use ($sheet_date, &$drug_infused) {
                    ...
                    $drug_infused[$sheet_date][$data->code][$prescription_id]['drug_name'] = $data->drug_name;
                    $drug_infused[$sheet_date][$data->code][$prescription_id]['instruction'] = $data->instruction;
                    $drug_infused[$sheet_date][$data->code][$prescription_id]['day_infused'] = array_sum(collect($prescription_data)->pluck('infused')->toArray());
                });
                ...
                ->map(function ($data, $id) use ($sheet_date, &$milk_volume) {
                    ...
                    if (!isset($milk_volume[$sheet_date])) {
                        $milk_volume[$sheet_date] = [];
                    }
                    $milk_volume[$sheet_date][$type][] = $vol;
                });
            ...
            foreach ($milk_volume as $sheet_date => $item) {
                foreach ($item as $key => $value) {
                    $day_wise_milk_volume[$sheet_date][$key] = array_sum($value);
                }
            }
        }

        return ['drug_infused' => $drug_infused, 'day_wise_milk_volume' => $day_wise_milk_volume, 'sheet_date_list' => $sheet_date_list];
