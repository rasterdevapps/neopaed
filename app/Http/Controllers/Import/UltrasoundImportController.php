...
class UltrasoundImportController extends Controller
{
    private function ultrasound() 
	{

		 $cultra = Excel::load(public_path().'/im/cranialultrasonography.xls', function ($reader) {
					         
					         $op_list = $reader->skipRows(1)->takeRows(1000)->get();
				   
		   })->all();

             foreach ($cultra as $key => $cultra_done) {
                	$neonatal_consultant[] = trim($cultra_done['cuss_done_by']);
               }  
		    ...
		    foreach ($cultra as $key => $op_value) {

			    	$baby=Baby::where('BMrNo', $op_value['mr_no.'])
	                      ->join('mother', 'mother.MotherId', '=', 'baby.MotherId')
	                      ->first();
                    $doctors = DoctorMaster::where('Name', 'like', '%'.$op_value['cuss_done_by'].'%')->first();
			     ...
			     $baby=Baby::where('BMrNo', $op_value['mr_no.'])
                      ...
                      ->first();	
						...
						$data['UsgRt']      	=	$op_value['usg_findings_rt_side'];
						$data['UsgLt']      	=	$op_value['usg_findings_lt_side'];
						$data['UsgGeneral']	    =	$op_value['usg_findings_general'];
						$data['Impression']	    =	$op_value['impression'];
						$data['Indication']	    =	$op_value['indication'];
