    ...
    public function getfhirjsondata(Request $request) 
    {
           ...
           if (!isset($machine_entry['entry'])) {
 		   ...
 		   foreach ($machine_entry as $machine_entry_key => $machine_entry_value) {
              ...
              if(isset($machine_entry_value->resource->resourceType) && $machine_entry_value->resource->resourceType == 'Device') {

                if (isset($machine_entry_value->resource->model)) {
                         $machine_data['device_type'] =   $machine_entry_value->resource->model;
                    }
                 }
	       ...
	       $machine_data['received_time']  = Carbon::now($this->time_zone);
	       $machine_data['is_parsed']      = false;
           $machine_data['is_lab_value']   = true;
	       $machine_data['received_ip']    = $request->ip(); 
           $machine_data['received_date']  = Carbon::now()->format('Y-m-d');

	  	   $id =  MachineData::create($machine_data)->id;
