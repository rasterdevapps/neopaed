...
class ProcedureController extends Controller 
{
	...
	{
		$this->middleware('role:MAS_B_PROCEDUURE,write', ['only'=>['store','update','edit','create','destroy']]);
		$this->middleware('role:MAS_B_PROCEDUURE,read', ['only'=>['index']]);	
		$this->auth = $auth;
	}
	...
	public function index(Request $request)
	{
	    ...
	    $result 	= ProcedureMaster::list($request->input('page'), $limit, $search_txt, $order, 1);
	    $results 	= $result['result'];

	    $getTotal   = ProcedureMaster::getCount();
	    $total   	= $result['total'];	
		...
		return view('masters.procedures.list', compact('results', 'pagination', 'search_txt', 'order', 'getTotal'));
	...
	{
		return view('masters.procedures.create');
	}
	...
	public function store(Request $request)
	{
		...
		foreach ($input['Name'] as $key => $value) {
			...
			ProcedureMaster::create($post);
		}
		return redirect(action('Masters\ProcedureController@index'))->with('Success', 'Record added successfully');
	}
	...
	public function edit($id)
	{
		...
		$results = ProcedureMaster::findOrfail($id);
		return view('masters.procedures.edit', compact('results'));
	}
	/**
	...
	public function update($id, Request $request)
	{
		...
		return redirect(action('Masters\ProcedureController@index'))->with('Success', 'Record updated successfully');
	}
	/**
	...
	*/
	public function destroy($id)
	{
		...
		$delete_data = array(
			'Name'		  	   => $results['Name'],
			'ModuleController' => 'Masters\ProcedureController',
			'ModuleId'		   => $id,
			'ModuleName'	   => 'Procedure Master',
			'UserDeleted'	   => $this->auth->user()->id,
			'DateDeleted'	   => Carbon::now()
		);
		DeleteApproval::create($delete_data);
		return redirect(action('Masters\ProcedureController@index'))->with('info', 'Record deleted successfully !');
	}
