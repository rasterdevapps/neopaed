...
use Carbon\Carbon;
use App\Models\Masters\ProblemMaster;
...
class ProblemController extends Controller 
{
	...
	{
		$this->middleware('role:MAS_B_PROBLEM,write', ['only'=>['store','update','edit','create','destroy']]);
		$this->middleware('role:MAS_B_PROBLEM,read', ['only'=>['index']]);	
		$this->auth = $auth;
	}
	...
	public function index(Request $request)
	{
	    ...
	    //get the problems record list form problems masters
	    $result 	= ProblemMaster::list($request->input('page'), $limit, $search_txt, $order, 1);
	    $results 	= $result['result'];

	    $getTotal   = ProblemMaster::getCount();	
	    $total   	= $result['total'];	
		...
		return view('masters.problems.list', compact('results', 'pagination', 'search_txt', 'order', 'getTotal'));
	...
	{
		//
		return view('masters.problems.create');
	}
	...
	public function store(Request $request)
	{
		...
		foreach ($input['Name'] as $key => $value) {
			...
			ProblemMaster::create($post);
		}
		
		return redirect(action('Masters\ProblemController@index'))->with('Success', 'Record added successfully');
	}
	...
	public function edit($id)
	{
		...
		$results = ProblemMaster::findOrfail($id);
		return view('masters.problems.edit', compact('results'));
	}
	/**
	...
	public function update($id, Request $request)
	{
		...
		return redirect(action('Masters\ProblemController@index'))->with('Success', 'Record updated successfully');
	}
	/**
	...
	*/
	public function destroy($id)
	{
		...
		$delete_data = array(
			'Name'		  	   => $results['Name'],
			'ModuleController' => 'Masters\ProblemController',
			'ModuleId'		   => $id,
			'ModuleName'	   => 'Problem Master',
			'UserDeleted'	   => $this->auth->user()->id,
			'DateDeleted'	   => Carbon::now()
		);
		DeleteApproval::create($delete_data);
		return redirect(action('Masters\ProblemController@index'))->with('info', 'Record deleted successfully !');
