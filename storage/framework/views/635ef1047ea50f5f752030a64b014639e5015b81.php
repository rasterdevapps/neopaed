...
<div class="crumbs bread-crumbs-shadow">
    <ul id="breadcrumbs" class="breadcrumb">
        ...
        <li class="current">
            <a href="<?php echo e(action('Ward\BabyWardController@index'), false); ?>">Ward Management</a>
        </li>
    </ul>
</div>
<!-- /Breadcrumbs line -->
    ...
    <div class="col-md-12">
        <div class="widget box">
            <div class="widget-content room-wise-list no-padding">
                <?php echo $__env->make('ward.bed-list', array_except(get_defined_vars(), array('__data', '__path')))->render(); ?>
            </div>
        </div>
    </div>
    <!-- /.col-md-12 -->
    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            ...
            <div class="row modal-body p-15">
                <?php echo Form::open(['url'=>action('Ward\BabyWardController@getaddbabytobed'), 'method'=>'GET','id'=>'registration-bed-list']); ?>

                <div class="col-md-12 pb-15">
                    <?php echo Form::label('admission_baby_id','Baby Name:'); ?>

                    <?php echo Form::select('admission_baby_id',[],null,['class'=>'form-control']); ?>

                </div>
                <div class="col-md-12">
                    <?php echo Form::label('admission_device_id','Device List:'); ?>

                    <?php echo Form::select('admission_device_id',DeviceHelpers::getdeviceList(),null,['class'=>'select2-select-00 full-width-fix','multiple']); ?>
                        ...
                        <div class="col-md-12">
                            <form id="quickreg">
                                ...
                                <div class="form-group"> 
                                    <label for="baby_mrn">Baby's <?php echo e(Lang::get('home.mrn'), false); ?></label>
                    ...
                    <div class="tab-pane" id="tab5">
                        <div class="col-md-12">
                            <form id="alreadyreg">
                                ...
                                <input type="hidden" name="hbed_name" class="form-control" data-value="" readonly required>
                                <div class="form-group"> 
                                    <label for="baby_mrn" class="required-label">Baby's <?php echo e(Lang::get('home.mrn'), false); ?></label>
                    ...
                    <div class="tab-pane" id="tab6">
                        <div class="col-md-12">
                            <form id="basicreg">
                                ...
                                <div class="form-group">
                                    <label for="mother_mrn">Mother <?php echo e(Lang::get('home.mrn'), false); ?></label>
                                ...
                                <div class="form-group"> 
                                    <label for="baby_ip_number" class="required-label">Baby's <?php echo e(Lang::get('home.ip'), false); ?></label>
...
function bedLogInterChange(babyIdone, admissionIdone, wardIdone, roomIdone, bedIdone, oldBedIdone, babyIdtwo, admissionIdtwo, wardIdtwo, roomIdtwo, bedIdtwo, oldBedIdtwo) {
    ...
    $.ajax({
        type: "GET",
        url: "<?php echo e(action('Ward\BabyWardController@getbabyinterchange'), false); ?>",
        data: {
            babyIdone: babyIdone,
...
$('.start-bed-regi').click(function(e) {
    ...
    if ($('#basicreg input[name="baby_mrn"]').val() == '') {
        $('#basicreg input[name="baby_mrn"]').parent().find('span').addClass('error-message').text('Please enter Baby\'s <?php echo e(Lang::get("home.mrn"), false); ?>');
        ...
        if ($('#basicreg input[name="baby_mrn"]').val().length != 6) {
            $('#basicreg input[name="baby_mrn"]').parent().find('span').addClass('error-message').text('Baby\'s <?php echo e(Lang::get("home.mrn"), false); ?> must be 6 characters');
            $('#basicreg input[name="baby_mrn"]').focus();
            return false;
        }
    }
    ...
    $.ajax({
        type: "GET",
        url: "<?php echo e(url('patient-status'), false); ?>",
        data: {
            baby_mrn: baby_mrn
        },
        success: function(response) {
            ...
            if (response.result == null) {
                ...
                if (ip_number == '') {
                    $('#basicreg input[name="baby_ip_number"]').parent().find('span').addClass('error-message').text('Please enter Baby\'s <?php echo e(Lang::get("home.ip"), false); ?>');
                    ...
                    if (ip_number.length < 7) {
                        $('#basicreg input[name="baby_ip_number"]').parent().find('span').addClass('error-message').text('Baby\'s <?php echo e(Lang::get("home.ip"), false); ?> must be 7 characters');
                        $('#basicreg input[name="baby_ip_number"]').focus();
                        return false;
                    }
                }
                ...
                window.location = "<?php echo e(action('Registration\NurseMotherController@create'), false); ?>";
            } else {
                if (typeof response.result.baby_id !== 'undefined' && response.result.baby_id != '' && response.result.baby_id != null) {
                    ...
                    else
                    {
                        ...
                        $.ajax({
                            type: "GET",
                            url: "<?php echo e(url('ward-basic-reg'), false); ?>",
                            data: {
                                baby_id: baby_id,
                                baby_mrn: baby_mrn,
...
$('#admitte-baby-bed').click(function() {
    ...
    $.ajax({
        type: "GET",
        url: "<?php echo e(action('Ward\BabyWardController@getaddbabytobed'), false); ?>",
        data: {
            babyid: babyid,
            wardid: wardid,
            roomid: roomid,
...
function getRemoveSyringePump(wardId, roomId, bedId, babyId, admissionId, slug) {
    ...
    $.ajax({
        type: "PATCH",
        url: "<?php echo e(action('Syringepump\SyrangePumpController@update', 0), false); ?>",
        data: {
            wardId: wardId,
            roomId: roomId,
            bedId: bedId,
...
function getRemoveInfusionPump(wardId, roomId, bedId, babyId, admissionId) {
    ...
    $.ajax({
        type: "PATCH",
        url: "<?php echo e(action('infusion\InfusionController@update', 0), false); ?>",
        data: {
            wardId: wardId,
            roomId: roomId,
            bedId: bedId,
...
function dischargeUrl(babyId, admissionId, ward_id) {
    ...
    $.ajax({
        type: "GET",
        url: "<?php echo e(url('discharge-baby'), false); ?>",
        data: {
            babyId: babyId,
            admissionId: admissionId,
            ward_id: ward_id
...
$('.list-chart-details').click(function() {
    ...
    var url = "<?php echo e(url('nurse-sheets-manual-graph'), false); ?>";
...
$('.list-details').click(function() {
    ...
    $.ajax({
        type: "GET",
        url: "<?php echo e(url('nurse-sheets-manual-print-latest'), false); ?>",
        data: {
            admissionId: admissionId
        },
        success: function(response) {
...
$('#search_input_text_field').keyup(function() {
    ...
    if (filter && filter.length > 0) {
        ...
        if (result_babies.length == 0) {
            result_babies = '<h3 class="text-center" style="margin-top:100px">No results found for this <?php echo e(Lang::get("home.mrn"), false); ?></h3>';
        }
...
$('#alreadyreg #visit-regi').on('click', function(e) {
    ...
    if ($('#alreadyreg input[name ="baby_mrn"]').val() == '') {
        $('#alreadyreg input[name ="baby_mrn"]').parent().find('span').addClass('error-message').text('Please enter Baby\'s <?php echo e(Lang::get("home.mrn"), false); ?>');
        ...
        if ($('#alreadyreg input[name ="baby_mrn"]').val().length != 6) {
            $('#alreadyreg input[name ="baby_mrn"]').parent().find('span').addClass('error-message').text('Baby\'s <?php echo e(Lang::get("home.mrn"), false); ?> must be 6 characters');
            $('#alreadyreg input[name ="baby_mrn"]').focus();
            return false;
        }
    }
    ...
    if (baby_mrn != '') {
        $.ajax({
            ...
            type: "GET",
            url: "<?php echo e(action('Ward\BabyWardController@getPatientStatus'), false); ?>",
            data: {
                baby_mrn: baby_mrn,
            },
            success: function(response) {
...
function hmsiprequest(current_element) {
    ...
    $.ajax({
        ...
        type: "POST",
        url: "<?php echo e(action('Fhir\HmsInterfacingController@registerPatientInHMS'), false); ?>",
        data: {
