<?php $site_url = url('/').'/public'; ?>
<script type="text/javascript">

$(document).ready(function(){
  
  	<?php if(@$results->Surfactant_therapy_nicu == 'No' || is_null(@$results->Surfactant_therapy_nicu)): ?>
      $('#Surfactant_therapy_nicu').bootstrapToggle('off');
	  <?php endif; ?>

    <?php if(@$results->Stools == 'Bowels not opened'  || is_null(@$results->Stools)): ?>
      $('#Stools').bootstrapToggle('off');
    <?php endif; ?>

  	<?php if(@$results->Hypoglycemia == 0 || is_null(@$results->Hypoglycemia)): ?>
         $('#Hypoglycemia').bootstrapToggle('off');
  	<?php endif; ?>

    <?php if(@$results->Hyperglycemia == 0  || is_null(@$results->Hyperglycemia)): ?>
        $('#Hyperglycemia').bootstrapToggle('off');
    <?php endif; ?>

    <?php if(@$results->InsulinTherapy == 0 || is_null(@$results->InsulinTherapy)): ?>
        $('#InsulinTherapy').bootstrapToggle('off');
    <?php endif; ?>

    <?php if(@$results->Hyponatremia == 0 || is_null(@$results->Hyponatremia)): ?>
        $('#Hyponatremia').bootstrapToggle('off');
    <?php endif; ?>

    <?php if(@$results->Hypernatremia==0 || is_null(@$results->Hypernatremia)): ?>
        $('#Hypernatremia').bootstrapToggle('off');
    <?php endif; ?>

    <?php if(@$results->Hypokalemia == 0 || is_null(@$results->Hypokalemia)): ?>
       $('#Hypokalemia').bootstrapToggle('off');
    <?php endif; ?>

    <?php if(@$results->Hyperkalemia==0 || is_null(@$results->Hyperkalemia)): ?>
       $('#Hyperkalemia').bootstrapToggle('off');
    <?php endif; ?>

    <?php if(@$results->Hypocalcemia==0 || is_null(@$results->Hypocalcemia)): ?>
       $('#Hypocalcemia').bootstrapToggle('off');
    <?php endif; ?>

    <?php if(@$results->Hypercalcemia==0 || is_null(@$results->Hypercalcemia)): ?>
       $('#Hypercalcemia').bootstrapToggle('off');
    <?php endif; ?>

    <?php if(@$results->intercostaldrain==1 || is_null(@$results->intercostaldrain)): ?>
       $('#intercostaldrain').bootstrapToggle('off');
    <?php endif; ?>
    <?php if(@$results->needlethoracentesis==1 || is_null(@$results->needlethoracentesis)): ?>
       $('#needlethoracentesis').bootstrapToggle('off');
    <?php endif; ?>
    <?php if(@$results->ultrasoundabdominal == 1 || is_null(@$results->ultrasoundabdominal)): ?>
           $('#ultrasoundabdominal').bootstrapToggle('off');
    <?php endif; ?>

    <?php if(@$results->renalultrasound ==1 || is_null(@$results->renalultrasound)): ?>
        $('#renalultrasound').bootstrapToggle('off');
    <?php endif; ?>
    <?php if(@$results->neuro_sonogram =='Not performed' || is_null(@$results->neuro_sonogram)): ?>
       $('#neuro_sonogram').bootstrapToggle('off');
    <?php endif; ?>
    <?php if(@$results->mrict_brain_status == 1 || is_null(@$results->mrict_brain_status)): ?>
       $('#mrict_brain_status').bootstrapToggle('off');
    <?php endif; ?>
    <?php if(@$results->viral_meningitis == 1 || is_null(@$results->viral_meningitis)): ?>
       $('#viral_meningitis').bootstrapToggle('off');
    <?php else: ?> 
        $('#viral_meningitis').bootstrapToggle('on');
    <?php endif; ?>
     <?php if(@$results->ultrasound_spine == 1 || is_null(@$results->ultrasound_spine)): ?>
       $('#ultrasound_spine').bootstrapToggle('off');
    <?php endif; ?>

    
    <?php if(@$results->eeg_cfm == 1 || is_null(@$results->eeg_cfm)): ?>
        $('#eeg_cfm').bootstrapToggle('off');
    <?php endif; ?>

    <?php if(@$results->dilution_exchange == 1 || is_null(@$results->dilution_exchange)): ?>
        $('#dilution_exchange').bootstrapToggle('off');
    <?php endif; ?>

    <?php if(@$results->chronic_lung == 1 || is_null(@$results->chronic_lung)): ?>
...
$('.send_message').click(function(){
  ...
  $.ajax({
         type:'GET',
         url:'<?php echo e(url("daycare-message-doctor"), false); ?>',
