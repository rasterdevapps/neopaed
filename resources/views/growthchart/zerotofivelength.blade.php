...
@php $boys_chart_length  = url('public/img/growth-chart/cht-lhfa-boys-p-0-5.svg'); @endphp
@php $girls_chart_length  = url('public/img/growth-chart/cht-lhfa-girls-p-0-5.svg'); @endphp
...
var gra_width = $('#{{$growth_chart_id}}').width();
if (gra_width > 769) {
	// $('#{{$growthcharts_parent}}').css('padding-top', '15px');
}
else{
	// $('#{{$growthcharts_parent}}').css('padding-top', '0px');
}
var sex = '{{$sex}}';
if(sex == 'Male') {
$('#{{$growthcharts_parent}}').addClass('who-growth-chart-male-length');
	// $('#{{$growthcharts_parent}}').css('background-color', '#0098DB');

}else if(sex == 'Female') {
$('#{{$growthcharts_parent}}').addClass('who-growth-chart-female-length');
	// $('#{{$growthcharts_parent}}').css('background-color', '#e45bbf');
...
function setLengthChartBackground(sourceId,sex,chartCharacter) {
    ...
    if(sex == 'Male') {
       ...
       // $('#'+sourceId).css('background-image','url("{{ $boys_chart_length }}") !important').css('background-color','#4D90FE');

       // $('#'+sourceId).attr('style', 'background-image: url("{{ $boys_chart_length }}") !important; background-color: #0098DB;');

    }else if(sex == 'Female') {
   
       // $('#'+sourceId).css('background-image','url("{{ $girls_chart_length }}") !important').css('background-color','#F390BC');
       // $('#'+sourceId).attr('style', 'background-image: url("{{ $girls_chart_length }}") !important; background-color: #e45bbf;');
    }
...
function growthChart(chartId,sex,chartCharacter){
		...
		//xAxis contain weight
         ...
         var xAxisLength                  = new AmCharts.ValueAxis();
		    ...
		    xAxisLength.maximum  	      = 120;
		    xAxisLength.minimum  	      = (sex == 'Female') ? 45 : 45;
		    xAxisLength.minorGridEnabled  = true;
		    xAxisLength.minorGridAlpha    = 0;
		    xAxisLength.minorTickLength   = 0;
		    ...
		    yAxisLength.gridCount        = 60;
		    yAxisLength.gridAlpha        = 0;
		    yAxisLength.maximum          = 60;
		...
		<?php 
          ...
          if ($baby_gestation > 36) {
               ?>
				cbabyGrowthlineLength.balloonText = "<span class='font-size-13'>Chronological Age : [[x8]] \n Length : [[value]]</span>";
               <?php
          }
          else
          {
