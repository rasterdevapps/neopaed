...
<head>
    ...
    <title>Postnatal Daycare Report</title>
    <style type="text/css">
        .print-tool-bar {
            float: right;
        }
...
<body>
    <div class="print-tool-bar">
        ...
        <a href="#" id="post-close" class="btn" title="Back">
            <i class="fa fa-times fa-2x" aria-hidden="true"></i>
        </a>
    </div>
    <div class="container">
        {!! Form::model(null,['method' => 'POST','url' => action('Admission\PostnatalDaycareController@saveFullEditor',null),'id'=>'post-daycare-report']) !!}
        {!! Form::hidden('daycare_id',$dischage_summary['daycare_id']) !!}
        <textarea name="post_daycare_report" id="post_daycare_report">
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            CKEDITOR.replace('post_daycare_report', {
                toolbar: [{
                    name: 'document',
                    items: ['Print']
                },
            ...
            CKEDITOR.instances["post_daycare_report"].on('blur',function() {
                postReportSave();  
            });
            $('.save-report').click(function() {
                $('#post-daycare-report').submit();
            });
            $(document).on('click', '#cke_16', function() {
                postReportSave();  
            });
            $('#post-close').click(function() {
                postReportSave(false);  
            });
            function postReportSave($slug=true) {
                var report=CKEDITOR.instances['post_daycare_report'].getData();
                $('textarea[name="post_daycare_report"]').val(report);
                ...
                $.ajax({
                    ...
                    type    :"POST",
                    url     : "{{ url('postnatal-daycare-reports-save') }}",
                    data    :$('#post-daycare-report').serialize(),
                    cache   : false,
                    dataType: "json",
                    success:function(response) {
                        if ($slug) {
                            ...
                            window.location = "{{ url('postnatal-daycare') }}";
                        }
                    },
