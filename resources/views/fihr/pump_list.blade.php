...
<div class="crumbs bread-crumbs-shadow">
	<ul id="breadcrumbs" class="breadcrumb">
		...
		<li class="current">
			<a href="{{ action('Fhir\FhirFormattedValuesController@pumpdata') }}">Pump Values</a>
		</li>                                         
	</ul>
</div>
		...
		<div class="widget box table-view-shadow">
			...
			<div class="widget-content">
					...
					<div class="row">
						<div class="dataTables_header clearfix">
							<div class="col-xs-4 col-sm-6 col-md-6">
								<div id="data-list_length" class="dataTables_length">
									<label class="data_limit">
										{{ Form::open(['url' => action('Fhir\FhirFormattedValuesController@pumpdata'), 'method' => 'get', 'id' => 'limit-form']) }}
							...
							<div class="col-xs-8 col-sm-6 col-md-6">
								...
								<!-- <button class="btn btn-success save-button-shadow ml-10 pull-right input-width-medium" id="oral-export">Export Oral Drugs</button> -->
								<!-- <button class="btn btn-success save-button-shadow ml-10 pull-right input-width-medium" id="non-oral-export">Export Non-Oral Drugs</button> -->
							</div>
						</div>
					</div>
				</div>
				<div class="col-md-12 mt-15 mb-20 p-10 multi-search display-grid" style="border: 1px solid #dddddd; float: unset;">
					{{ Form::model($search, ['url' => action('Fhir\FhirFormattedValuesController@pumpdata'), 'method' => 'get', 'id' => 'filter-pump']) }}
					<div class="col-md-2">
					...
					<div class="col-md-2" style="display: flex;flex-direction: row;justify-content: center;align-items: center;height: 70px;">
						<button class="btn btn-primary save-button-shadow pump-data-filter mr-10"> Search </button>
				...
				<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 text-center pb-5" style="background-color: #00ff4e38;">
					...
					<h3><strong>{{ $details->BabyName }} - {{ $details->BMrNo }} ( {{ isset($details->DOB) ? '/ ' . date('d-m-Y', strtotime($details->DOB)) : '' }} )</strong></h3>
				</div>
				@endif
				<table class="table table-striped table-bordered table-responsive" id="data-list">
					<thead>
						<tr>
							...
							<th class="sorting_by sorting_icon @if($order['sortby'] == 'result_time') sorting_{{$order['sortorder']}}_icon @endif" data-field="{{ SiteHelpers::encrypt_id('result_time') }}">Received Time</th>
							<th class="sorting_by sorting_icon @if($order['sortby'] == 'prescription_hdr.brand_name') sorting_{{$order['sortorder']}}_icon @endif" data-field="{{ SiteHelpers::encrypt_id('prescription_hdr.brand_name') }}">Brand / Pharmacological Name</th>
							...
							<th class="sorting_by sorting_icon @if($order['sortby'] == 'remain_volume') sorting_{{$order['sortorder']}}_icon @endif" data-field="{{ SiteHelpers::encrypt_id('remain_volume') }}">Remaining volume</th>
							...
							<th class="sorting_by sorting_icon @if($order['sortby'] == 'pressure') sorting_{{$order['sortorder']}}_icon @endif" data-field="{{ SiteHelpers::encrypt_id('pressure') }}">Running Pressure</th>
					...
					<tbody>
						...
						<tr class="detailinfo">
							...
							<td><a href="{{ url('fhir-format-value'.'/'.SiteHelpers::encrypt_id($list->prescription_table_id)) }}/pump:{{$list->status}}">{{ date('d-m-Y H:i',strtotime($list->result_time)) }}</a></td>
							<td>{{ $list->brand_name . ' / ' . $list->generic_pharmacological_name }}</td>
							<td>{{ number_format($list->infused, 2) }}</td>
							<td>{{ number_format($list->prescription_rate, 2) }}</td>
							<td>{{ number_format($list->remain_volume, 2) }}</td>
							<td>{{ $list->remain_time }}</td>
							<td>{{ number_format($list->pressure, 2) }}</td>
							<td>{{ $list->pressure_level }}</td>
						</tr>                        	
						@php $i = $i+1 @endphp 
						@endforeach
						@else 
...
<script type="text/javascript">
	$(document).ready(function() {
		...
		$(".pump-data-filter").on("click", function() {
			$("#filter-pump").validate({
				rules:{
					mrno:{
						required: true
					}
	    ...
	    $('#oral-export').click(function() {
	        var babyMrn = $('#mrno').val();
	        var babyIpNumber = $('#ipnumber').val();
	        window.location = "{{ url('oral-drugs-export') }}" + '?mrn=' + babyMrn + '&ipnumber=' + babyIpNumber + '&type=oral';
	    });
	    $('#non-oral-export').click(function() {
	        var babyMrn = $('#mrno').val();
	        var babyIpNumber = $('#ipnumber').val();
	        window.location = "{{ url('oral-drugs-export') }}" + '?mrn=' + babyMrn + '&ipnumber=' + babyIpNumber + '&type=nonoral';
	    });
		...
		function pagination(sortby) {
			...
			var sorting_param  = $('#limit-form').serialize();
			var searchinput  = $('#filter-pump').serialize();
