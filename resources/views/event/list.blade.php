...
<div class="crumbs bread-crumbs-shadow">
  <ul id="breadcrumbs" class="breadcrumb">
    ...
    <li class="current">
      <a href="{{ action('Nurse\DashboardEventController@index') }}">{{ Lang::get('menu.side_menu_nicu_care_events') }} List</a>
    </li>                                                
  </ul>         
</div>
<!-- /Breadcrumbs line -->
  ...
  <div class="col-xs-12 col-sm-12 col-md-12">
    <div class="widget box table-view-shadow no-padding">
     <div class="widget-header">
       <h4>{{ Lang::get('menu.side_menu_nicu_care_events') }} List</h4>
     </div>
     <div class="widget-content">
      <div id="data-list_wrapper" class="dataTables_wrapper form-inline" role="grid">
        <div class="row">
          <div class="dataTables_header clearfix">
            <div class="col-xs-4 col-sm-6 col-md-6">
              <div id="data-list_length" class="dataTables_length">
                <label class="data_limit">
                  {!! Form::open(['url' => action('Nurse\DashboardEventController@index'), 'method' => 'get', 'id' => 'limit-form']) !!}
            ...
            {!! Form::open(['url' => action('Nurse\DashboardEventController@index'), 'method' => 'get', 'id' => 'search-form','class' => 'search_form']) !!}
            <div class="col-xs-8 col-sm-6 col-md-4 pull-right"> 
              <div class="input-group">
                ...
                <a href="{{ action('Nurse\DashboardEventController@index') }}" class="input-group-addon">
      ...
      <tbody>
       ...
       <tr >
        ...
        <td>
          <a class="btn btn-xs btn-success" href="{{ url('/') . '/get-events/'.\SiteHelpers::encrypt_id($results[$i]->BMrNo).'/1' }}"><i class="fa fa-sort-amount-asc" aria-hidden="true" style="transform: rotate(180deg); font-size: 10px;"></i> <span class="hidden-xs">View</span></a>
        </td>
        <td>
          <a class="btn btn-xs btn-info" href="{{ action('Nurse\DashboardEventController@chart').'?mrn='.SiteHelpers::encrypt_id($results[$i]->BMrNo) }}"><i class="fa fa-sort-amount-asc" aria-hidden="true" style="transform: rotate(180deg); font-size: 10px;"></i> <span class="hidden-xs">View</span></a>
        </td>
      </tr>
      @endfor
      @else
          ...
          <div class="dataTables_paginate paging_bootstrap pagination_footer">
            <ul class="pagination">    
              <li class="prev @if(Request::query('page') == '' || Request::query('page') == $pagination['start']) disabled @endif">
                <a class="@if(Request::query('page') != $pagination['start'] &&  Request::query('page') != '') sort_with_page @endif" href="@if(Request::query('page') == $pagination['start'] || Request::query('page') == '') javascript:void(0); @else {{ url('care-event-list?page='.$pagination['previous']) }}@endif">&#8592; Previous</a>
              </li>
              @for ($i = $pagination['start']; $i <= $pagination['end']; $i++) 
              <li class="@if($i == Request::query('page')) active @elseif(Request::query('page') == '' && $i == $pagination['start']) active @endif">
                <a class='sort_with_page' href="{{ url('care-event-list?page='.$i) }}">{{$i}}</a>
              </li>  
              @endfor
              <li class="next @if(Request::query('page') == $pagination['end'] || $pagination['total'] <= $pagination['limits']) disabled @endif">
                <a class="@if(Request::query('page') != $pagination['end'] && $pagination['start'] != $pagination['end']) sort_with_page @endif" href="@if(Request::query('page') == $pagination['end'] || $pagination['total'] <= $pagination['limits']) javascript:void(0); @else {{ url('care-event-list?page='.$pagination['next']) }}@endif">Next &#8594; </a>  
                </li>
              </ul>
            </div>
          </div>
