...
<div class="crumbs bread-crumbs-shadow">
	<ul id="breadcrumbs" class="breadcrumb">
		...
		<li class="current"><a href="{{ action('Settings\UsergroupController@index') }}">Usergroups</a></li>                                                
	</ul>
</div>
<!-- /Breadcrumbs line -->
<!--=== Page Content ===-->
	...
	<div class="col-md-12">
		<div class="widget box table-view-shadow">
			<div class="widget-header">
				<h4>Usergroups</h4>
                @if(in_array('USER_GROUPS',$write_permission))
                <a href="{{ action('Settings\UsergroupController@create') }}" title="" class="btn btn-basic-shadow btn-info pull-right create-btn-spacing"><i class="fa fa-plus "></i> <span>Create New</span></a>   
                @endif                             
            </div>
                ...
                <div id="data-list_wrapper" class="dataTables_wrapper form-inline" role="grid">
                    <div class="row">
                        <div class="dataTables_header clearfix" >
                            <div class="col-xs-4 col-sm-6 col-md-6">
                                <div id="data-list_length" class="dataTables_length">
                                    <label class="data_limit">
                                        {!! Form::open(['url' => action('Settings\UsergroupController@index'), 'method' => 'get', 'id' => 'limit-form']) !!}
                            ...
                            {!! Form::open(['url' => action('Settings\UsergroupController@index'), 'method' => 'get', 'id' => 'search-form','class' => 'search_form']) !!} 
                            <div class="col-xs-8 col-sm-6 col-md-4 pull-right">
                                <div class="input-group">
                                    ...
                                    <a href="{{ action('Settings\UsergroupController@index') }}" class="input-group-addon">
                ...
                <table class="table table-striped table-bordered table-responsive">
                    <thead>
                        <tr>
                          ...
                          @endif
                            @if(in_array('USER_GROUPS',$delete_permission))
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td class="center-align-phone">
                              <a class="btn btn-info btn-view" href="{{ action('Settings\UsergroupController@edit',SiteHelpers::encrypt_id($results[$i]->RoleId)) }}" title="Edit Record">
                               <i class="fa fa-edit"></i>
                           </a>
                      </td>
                          @endif
                            @if(in_array('USER_GROUPS',$delete_permission))
                           @if($results[$i]->RoleId != 1)
                            <td class="center-align-phone">
                           <a class="icon btn btn-danger btn-remove" href="{{ action('Settings\UsergroupController@destroy',$results[$i]->RoleId) }}" title="Delete Record">
                              <i class="fa fa-trash"></i> 
                          </a>
                      </td>
                          @else
                            <td class="center-align-phone">
                          <a class="icon btn btn-danger btn-remove disabled" href="javascript:" title="Delete Record">
                              <i class="fa fa-trash"></i> 
                          </a>
                      </td>
                          @endif
                        ...
                        <div class="dataTables_paginate paging_bootstrap pagination_footer">
                            <ul class="pagination">    
                                <li class="prev @if(Request::query('page') == '' || Request::query('page') == $pagination['start']) disabled @endif">
                                    <a class="@if(Request::query('page') != $pagination['start'] &&  Request::query('page') != '') sort_with_page @endif" href="@if(Request::query('page') == $pagination['start'] || Request::query('page') == '') javascript:void(0); @else {{ url('usergroups?page='.$pagination['previous']) }}@endif">&#8592; Previous</a>
                                </li>
                                @for ($i = $pagination['start']; $i <= $pagination['end']; $i++) 
                                <li class="@if($i == Request::query('page')) active @elseif(Request::query('page') == '' && $i == $pagination['start']) active @endif">
                                    <a class='sort_with_page' href="{{ url('usergroups?page='.$i) }}">{{$i}}</a>
                                </li>  
                                @endfor
                                <li class="next @if(Request::query('page') == $pagination['end'] || $pagination['total'] <= $pagination['limits']) disabled @endif">
                                    <a class="@if(Request::query('page') != $pagination['end'] && $pagination['start'] != $pagination['end']) sort_with_page @endif" href="@if(Request::query('page') == $pagination['end'] || $pagination['total'] <= $pagination['limits']) javascript:void(0); @else {{ url('usergroups?page='.$pagination['next']) }}@endif">Next &#8594; </a>  
                                </li>
                            </ul>
                        </div>
                    </div>
