...
<div class="crumbs bread-crumbs-shadow">
  <ul id="breadcrumbs" class="breadcrumb">
     ...
     <li class="current"><a href="{{ action('Registration\PediatricOpController@index') }}">Pediatric OP Registration</a></li>                                                
 </ul>                  
</div>
<!-- /Breadcrumbs line -->
<!--=== Page Content ===-->
    ...
    <div class="col-md-12">
        <div class="widget box table-view-shadow">
         <div class="widget-header">
            ...
            @if(in_array('PEDIATRICS_OP_REG',$write_permission))
                    <a href="{{ action('Registration\PediatricOpController@chooseBaby') }}" title="Create New" class="btn btn-basic-shadow create-btn-spacing btn-info pull-right create-btn"><i class="fa fa-plus "></i> <span>Create New</span></a> 
            @endif                               
        </div>
        <div class="widget-content">
            <div id="data-list_wrapper" class="dataTables_wrapper form-inline" role="grid">
                <div class="row">
                    <div class="dataTables_header clearfix">
                        <div class="col-xs-4 col-sm-6 col-md-6">
                            <div id="data-list_length" class="dataTables_length">
                                <label class="data_limit">
                                    {!! Form::open(['url' => action('Registration\PediatricOpController@index'), 'method' => 'get', 'id' => 'limit-form']) !!}
                        ...
                        {!! Form::open(['url' => action('Registration\PediatricOpController@index'), 'method' => 'get', 'id' => 'search-form','class' => 'search_form']) !!} 
                        <div class="col-xs-8 col-sm-4 col-md-4 pull-right">  
                            ...
                            <a href="{{ action('Registration\PediatricOpController@index')}}" class="input-group-addon"><i class="glyphicon glyphicon-remove"></i></a>
                        </div>
                        {!! Form::close() !!}
                    </div>
                </div>
            ...
            <table class="table table-striped table-bordered table-responsive"  id="data-list">
               <thead>
                <tr>
                    <th colspan="3" style="background-color: lightcoral;">{{ ValuelistHelpers::mas_doctors_list(8) }}</th>
                </tr>
                <tr>
            ...
            <tbody>
                @if(count($results) > 0 )
                @php 
                    $opdate = $results->unique('op_date')->pluck('op_date')->toArray();
                @endphp
                @foreach($opdate as $op_date)
                    ...
                    @php 
                        $result = $results->where('op_date',$op_date);
                    @endphp
                    @foreach($result as $result_details)
                        <tr>
                            <td class="text-captialize">
                                <a href="{{ action('Registration\PediatricOpController@OpsubList',SiteHelpers::encrypt_id($result_details->baby_id)) }}?seen_by={{SiteHelpers::encrypt_id(8)}}">{{  $result_details->BabyName }}</a>
                            </td>
                        ...
                        <div class="dataTables_paginate paging_bootstrap pagination_footer">
                            <ul class="pagination">
                                <li class="prev @if(Request::query('page') == '' || Request::query('page') == $pagination['start']) disabled @endif">
                                    <a class="@if(Request::query('page') != $pagination['start'] &&  Request::query('page') != '') sort_with_page @endif" href="@if(Request::query('page') == $pagination['start'] || Request::query('page') == '') javascript:void(0); @else {{ url('pediatric-out-patient?page='.$pagination['previous']) }}@endif">&#8592; Previous</a>
                                </li>
                                @for ($i = $pagination['start']; $i <= $pagination['end']; $i++)
                                <li class="@if($i == Request::query('page')) active @elseif(Request::query('page') == '' && $i == $pagination['start']) active @endif">
                                    <a class='sort_with_page' href="{{ url('pediatric-out-patient?page='.$i) }}">{{$i}}</a>
                                </li>
                                @endfor
                                <li class="next @if(Request::query('page') == $pagination['end'] || $pagination['total'] <= $pagination['limits']) disabled @endif">
                                    <a class="@if(Request::query('page') != $pagination['end'] && $pagination['start'] != $pagination['end']) sort_with_page @endif" href="@if(Request::query('page') == $pagination['end'] || $pagination['total'] <= $pagination['limits']) javascript:void(0); @else {{ url('pediatric-out-patient?page='.$pagination['next']) }}@endif">Next &#8594; </a>
                                </li>
                            </ul>
                        </div>
                    </div>
