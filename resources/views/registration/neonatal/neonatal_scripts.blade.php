...
@php $results = (isset($baby_detail)) ? $baby_detail : $results; @endphp
<script type="text/javascript">


  @if(@$results->Booked=='No' || is_null(@$results->Booked) || empty(@$results->Booked))
  $('#Booked').bootstrapToggle('off');
  @elseif(@$results->Booked== "Yes")
  $('#Booked').bootstrapToggle('on');
  @endif


  @if(@$results->Supervised == 'No' || is_null(@$results->Supervised) || empty(@$results->Supervised))
  $('#Supervised').bootstrapToggle('off');
  @elseif(@$results->Supervised == "Yes")
  $('#Supervised').bootstrapToggle('on');
  @endif


  @if(@$results->MultiplePregnancy == 'No' || is_null(@$results->MultiplePregnancy) || empty(@$results->MultiplePregnancy))
  $('#MultiplePregnancy').bootstrapToggle('off');
  @elseif(@$results->MultiplePregnancy == "Yes")
  $('#MultiplePregnancy').bootstrapToggle('on');
  @endif


  @if(@$results->PregnancyComplications == 'No' || is_null(@$results->PregnancyComplications) || empty(@$results->PregnancyComplications))
     $('#PregnancyComplications').bootstrapToggle('off');
  @elseif(@$results->PregnancyComplications == "Yes")
  $('#PregnancyComplications').bootstrapToggle('on');
  @endif

  @if(@$results->AntenatalSteroids == 'No' || is_null(@$results->AntenatalSteroids) || empty(@$results->AntenatalSteroids))
  $('#AntenatalSteroids').bootstrapToggle('off');
  @elseif(@$results->AntenatalSteroids == "Yes")
  $('#AntenatalSteroids').bootstrapToggle('on');
  @endif

  @if(@$results->Labour == 'No' || is_null(@$results->Labour) || empty(@$results->Labour))
  $('#Labour').bootstrapToggle('off');
  @elseif(@$results->Labour == "Yes")
  $('#Labour').bootstrapToggle('on');
  @endif

  @if(@$results->MaternalPyrexia == 'No' || is_null(@$results->MaternalPyrexia) || empty(@$results->MaternalPyrexia))
  $('#MaternalPyrexia').bootstrapToggle('off');
  @elseif(@$results->MaternalPyrexia == "Yes")
  $('#MaternalPyrexia').bootstrapToggle('on');
  @endif

  @if(@$results->PROM == 'No' || is_null(@$results->PROM) || empty(@$results->PROM))
  $('#PROM').bootstrapToggle('off');
  @elseif(@$results->PROM == "Yes")
  $('#PROM').bootstrapToggle('on');
  @endif

  @if(@$results->Resuscitation == 'No' || is_null(@$results->Resuscitation) || empty(@$results->Resuscitation))
   $('#Resuscitation').bootstrapToggle('off');
  @elseif(@$results->Resuscitation == "Yes") 
   $('#Resuscitation').bootstrapToggle('on');
  @endif

  @if(@$results->FacialOxygen == 'No' || is_null(@$results->FacialOxygen) || empty(@$results->FacialOxygen))
   $('#FacialOxygen').bootstrapToggle('off');
  @elseif(@$results->FacialOxygen == 'Yes') 
   $('#FacialOxygen').bootstrapToggle('on');
  @endif

  @if(@$results->Intubation == 'No' || is_null(@$results->Intubation) || empty(@$results->Intubation))
   $('#Intubation').bootstrapToggle('off');
  @elseif(@$results->Intubation == 'Yes') 
   $('#Intubation').bootstrapToggle('on');
  @endif

  @if(@$results->PPV == 'No' || is_null(@$results->PPV) || empty(@$results->PPV))
   $('#PPV').bootstrapToggle('off');
  @elseif(@$results->PPV == 'Yes') 
    $('#PPV').bootstrapToggle('on');
  @endif

  @if(@$results->CPR == 'No' || is_null(@$results->CPR) || empty(@$results->CPR))
   $('#CPR').bootstrapToggle('off');
  @elseif(@$results->CPR == 'Yes') 
   $('#CPR').bootstrapToggle('on'); 
  @endif

  @if(@$results->Drugs == 'No' || is_null(@$results->Drugs) || empty(@$results->Drugs))
   $('#Drugs').bootstrapToggle('off');
  @elseif(@$results->Drugs == 'Yes') 
   $('#Drugs').bootstrapToggle('on'); 
  @endif

  @if(@$results->Consanguinity =='No' || is_null(@$results->Consanguinity) || empty(@$results->Consanguinity))
   $('#Consanguinity').bootstrapToggle('off');
  @elseif(@$results->Consanguinity == 'Yes') 
   $('#Consanguinity').bootstrapToggle('on');     
  @endif

  @if(@$results->delivery_room_cpap =='No' || is_null(@$results->delivery_room_cpap) || empty(@$results->delivery_room_cpap))
   $('#delivery_room_cpap').bootstrapToggle('off');
  @elseif(@$results->delivery_room_cpap == 'Yes') 
   $('#delivery_room_cpap').bootstrapToggle('on');     
  @endif

  @if(@$results->bag_mask_ventilator =='No' || is_null(@$results->bag_mask_ventilator) || empty(@$results->bag_mask_ventilator))
   $('#bag_mask_ventilator').bootstrapToggle('off');
  @elseif(@$results->bag_mask_ventilator == 'Yes') 
   $('#bag_mask_ventilator').bootstrapToggle('on');     
  @endif

  @if(@$results->bag_mask_ventilator_duration =='Unknown' || is_null(@$results->bag_mask_ventilator_duration) || empty(@$results->bag_mask_ventilator_duration))
   $('#bag_mask_ventilator_duration').bootstrapToggle('off');
  @elseif(@$results->bag_mask_ventilator_duration == 'known') 
  ...
  @if(@$results->adjustedtrisomies == 2)
   $('#adjustedtrisomies').bootstrapToggle('on');
   
       @if(!empty($results->AdjustedRiskForTrisomy21))
        $('#AdjustedRiskForTrisomy21').val('{{ $results->AdjustedRiskForTrisomy21 }}');
       @endif

       @if(!empty($results->AdjustedRiskForTrisomy18))
          $('#AdjustedRiskForTrisomy18').val('{{ $results->AdjustedRiskForTrisomy18 }}');
       @endif

       @if(!empty($results->AdjustedRiskForTrisomy13))
          $('#AdjustedRiskForTrisomy13').val('{{ $results->AdjustedRiskForTrisomy13 }}');
       @endif

  @endif

@if(@$results->timeofgasp_status == true)
   $('#timeofgasp_status').bootstrapToggle('on');
@endif  

@if(@$results->regularrespiration_status == true)
   $('#regularrespiration_status').bootstrapToggle('on');
@endif

@if(@$results->insertion_status == true)
   $('#insertion_status').bootstrapToggle('on');
@else 
 $('#insertion_status').bootstrapToggle('off');
@endif

@if(@$results->ppv_status == true)
   $('#ppv_status').bootstrapToggle('on');
@endif

@if(@$results->cpr_status == true)
