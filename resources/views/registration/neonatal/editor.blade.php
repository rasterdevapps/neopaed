...
<body>
    <div class="print-tool-bar">
        ...
        <a href="#" id="neonatal-close" class="btn" title="Back">
            <i class="fa fa-times fa-2x" aria-hidden="true"></i>
        </a>
    </div>
    <div class="container">
        {!! Form::model(null,['method' => 'POST','url' => action('Registration\NeonatalController@saveFullEditor',null),'id'=>'neonatal-reports']) !!}
        {!! Form::hidden('baby_id',$dischage_summary['baby_id']) !!}
        {!! Form::hidden('neonatal_id',$dischage_summary['neonatal_id']) !!}
        <textarea name="neonatal_report" id="neonatal_report">
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            CKEDITOR.replace('neonatal_report', {
                toolbar: [{
                    name: 'document',
                    items: ['Print']
                },
            ...
            CKEDITOR.instances["neonatal_report"].on('blur',function() {
                neonatalReportSave();
            });
            $('.save-report').click(function() {
                $('#neonatal-reports').submit();
            });
            $(document).on('click', '#cke_16', function() {
                neonatalReportSave();
            });
            $('#neonatal-close').click(function() {
                neonatalReportSave(false);  
            });
            function neonatalReportSave(slug=true) {
                    var report=CKEDITOR.instances['neonatal_report'].getData();
                    $('textarea[name="neonatal_report"]').val(report);
                    ...
                    $.ajax({
                        ...
                        type    :"POST",
                        url     : "{{ url('neonatal-reports-save') }}",
                        data    :$('#neonatal-reports').serialize(),
                        cache   : false,
                        dataType: "json",
