...
<!-- Breadcrumbs line -->
	<div class="crumbs bread-crumbs-shadow">
		<ul id="breadcrumbs" class="breadcrumb">
			...
			<li class="current"><a href="{{ action('Registration\NeuroController@index') }}">Neuro Development</a></li>                                                
		</ul>		          
         <ul class="pull-right list-none hide">
            <li>
                <a href="{{ action('Search\NeuroController@create') }}" title="search" class="btn btn-info btn-basic-shadow create-btn-spacing pull-right mr-33 search-btn">
        ...
        <div class="widget box table-view-shadow">
			<div class="widget-header">
                ...
                @if(in_array('NEURO_DEVELOPMENT',$write_permission))
                    <!-- <a href="{{ action('Registration\NeuroController@create').'/'.SiteHelpers::encrypt_id(0) }}" title="Create New" class="btn btn-basic-shadow create-btn-spacing btn-info pull-right create-btn"><i class="fa fa-plus"></i><span>Create New</span></a>   -->
                    <a href="{{ action('Registration\NeuroController@chooseBaby') }}" title="Create New" class="btn btn-basic-shadow create-btn-spacing btn-info pull-right create-btn"><i class="fa fa-plus"></i><span>Create New</span></a>                      
                @endif                               
			</div>
                ...
                <div id="data-list_wrapper" class="dataTables_wrapper form-inline" role="grid">
                    <div class="row">
                        <div class="dataTables_header clearfix">
                            <div class="col-xs-4 col-sm-6 col-md-6">
                                <div id="data-list_length" class="dataTables_length">
                                    <label class="data_limit">
                                        {!! Form::open(['url' => action('Registration\NeuroController@index'), 'method' => 'get', 'id' => 'limit-form']) !!}
                            ...
                            {!! Form::open(['url' => action('Registration\NeuroController@index'), 'method' => 'get', 'id' => 'search-form','class' => 'search_form']) !!} 
                              <div class="col-xs-8 col-sm-4 col-md-4 pull-right">  
                                <div class="input-group">
                                  ...
                                  <input type="text" aria-controls="data-list" class="form-control" name="search_txt" value="{{  @$search['search_txt'] }}" placeholder="Search">
                                     <a href="{{ action('Registration\NeuroController@index')}}" class="input-group-addon"><i class="glyphicon glyphicon-remove"></i></a>
                                </div>
                        {!! Form::close() !!}
                         </div>
                    </div>
                    ...
                    <tbody>
                        @if(count($results) > 0)
                            @foreach($results as $date => $result)
                                    <tr>
                                        <td colspan="3" style="background-color: #00c3ff;"><h3 class="m-0"><b>{{ date('d-m-Y', strtotime($date)) }}</b></h3></td>
                                    </tr>
                                @foreach($result as $key => $value)
                                    <tr>
                                        <td class="text-captialize">
                                            <a href="{{ action('Registration\NeuroController@NeuroSubList',SiteHelpers::encrypt_id($value->BabyId)) }}">{{  $value->BabyName }}</a>
                                        </td>
                                        <td>{{  $value->BMrNo }}</td>
                                        <td>{{  (date('Y',strtotime($value->DOB)) > 1970) ? date('d-m-Y',strtotime($value->DOB)) : ''  }}</td>
                                    </tr>
                                @endforeach
                            @endforeach
                        @else
                                ...
                                <div class="dataTables_paginate paging_bootstrap pagination_footer">
                                    <ul class="pagination">
                                        <li class="prev @if(Request::query('page') == '' || Request::query('page') == $pagination['start']) disabled @endif">
                                            <a class="@if(Request::query('page') != $pagination['start'] &&  Request::query('page') != '') sort_with_page @endif" href="@if(Request::query('page') == $pagination['start'] || Request::query('page') == '') javascript:void(0); @else {{ url('neuro-develop?page='.$pagination['previous']) }}@endif">&#8592; Previous</a>
                                        </li>
                                        @for ($i = $pagination['start']; $i <= $pagination['end']; $i++)
                                        <li class="@if($i == Request::query('page')) active @elseif(Request::query('page') == '' && $i == $pagination['start']) active @endif">
                                            <a class='sort_with_page' href="{{ url('neuro-develop?page='.$i) }}">{{$i}}</a>
                                        </li>
                                        @endfor
                                        <li class="next @if(Request::query('page') == $pagination['end'] || $pagination['total'] <= $pagination['limits']) disabled @endif">
                                            <a class="@if(Request::query('page') != $pagination['end'] && $pagination['start'] != $pagination['end']) sort_with_page @endif" href="@if(Request::query('page') == $pagination['end'] || $pagination['total'] <= $pagination['limits']) javascript:void(0); @else {{ url('neuro-develop?page='.$pagination['next']) }}@endif">Next &#8594; </a>
                                        </li>
                                    </ul>
                                </div>
                            </div>
