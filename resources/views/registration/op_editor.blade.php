...
<body>
    <div class="print-tool-bar">
        ...
        <a href="#" id="op-close" class="btn" title="Back">
            <i class="fa fa-times fa-2x" aria-hidden="true"></i>
        </a>
    </div>
    <div class="container">
        {!! Form::model(null,['method' => 'POST','url' => action('Registration\OpController@saveFullEditor',null),'id'=>'op-reports']) !!}
        {!! Form::hidden('op_id',$dischage_summary['op_id']) !!}
        <textarea name="op_report" id="op_report">
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            ...
            CKEDITOR.instances["op_report"].on('blur',function() {
                opReportSave();
            });
            ...
            function opReportSave($slug=true) {
                var report=CKEDITOR.instances['op_report'].getData();
                $('textarea[name="op_report"]').val(report);
                ...
                $.ajax({
                    ...
                    type    :"POST",
                    url     : "{{ url('op-reports-save') }}",
                    data    :$('#op-reports').serialize(),
                    cache   : false,
                    dataType: "json",
                    success:function(response) {
                        if ($slug) {
                            ...
                            window.location = "{{ url('out-patient') }}";                        
                        }
                    },
