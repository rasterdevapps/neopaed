		        ...
		        <table class="table table-striped table-bordered table-responsive"  id="data-list">
					<thead>
                        <tr>
                            ...
                            <th class="sorting_by sorting_icon @if($order['sortby'] == 'DOB') sorting_{{$order['sortorder']}}_icon @endif" data-field="{{ SiteHelpers::encrypt_id('DOB') }}" data-hide="phone,tablet">Date of birth</th>
                        </tr>
                    </thead>
                    <tbody>
                        @if(count($results) > 0 )
                         @php $opdate =$results->unique('OpDate')->pluck('OpDate')->toArray();   @endphp
                         @foreach($opdate as $op_date)
                         @php $result = $results->where('OpDate',$op_date);  @endphp
                            <tr>
                                <td colspan="3" style="background-color: #00c3ff;"><b>{{ date('d-m-Y',strtotime($op_date))  }}</b></td>
                            </tr>
                            @foreach($result as $op_result)
                                <tr>
                                    <td class="text-captialize"><a href="{{ action('Registration\OpController@OpsubList',SiteHelpers::encrypt_id($op_result->BabyId)) }}">{{  $op_result->BabyName }}</a></td>
                                    <td>{{  $op_result->BMrNo }}</td>
                                    <td>{{  (date('Y',strtotime($op_result->DOB)) > 1970) ? date('d-m-Y',strtotime($op_result->DOB)) : ''  }}</td>
                                </tr>
                        	@endforeach
                        @endforeach    
                        @else
