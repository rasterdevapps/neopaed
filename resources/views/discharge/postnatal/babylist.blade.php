...
@endphp 
<!-- Breadcrumbs line -->
<div class="crumbs bread-crumbs-shadow discharge-post">
	<ul id="breadcrumbs" class="breadcrumb">
	 ...
	 <ul class="pull-right postnatal discharge-color-code list-none display-flex mt-10">
	    <li><span class="info-tick info"></span> <b>Inpatient</b></li>
	    <li><span class="tick success"></span>  <b>Discharged or Died</b></li>
	 </ul>
    <ul class="pull-right nicu-list discharge-color-code1 list-none hidden-xs hidden-sm mt-5">
        <li>
            <a class='btn btn-info btn-basic-shadow search-btn' href="{{ action('Search\PostnatalController@create').'?module=discharge' }}">
        ...
        <div class="widget box table-view-shadow">
			<div class="widget-header">
			    ...
			    @if(in_array('MOTHER_REG',$write_permission))
					   <a href="{{ action('Admission\PostnatalController@create') }}" title="create new" class="btn hide create-btn-spacing  btn-info btn-basic-shadow pull-right">
       			...
       			<div id="data-list_wrapper" class="dataTables_wrapper form-inline" role="grid">
                    <div class="row">
			            <div class="dataTables_header clearfix" >
			                <div class="col-xs-4 col-sm-6 col-md-6">
				                <div id="data-list_length" class="dataTables_length ">
				                    <label class="data_limit">
				                      {!! Form::open(['url' => action('Admission\PostnatalDischargeController@index'), 'method' => 'get', 'id' => 'limit-form']) !!}
		                 ...
		                 {!! Form::open(['url' => action('Admission\PostnatalDischargeController@index'), 'method' => 'get', 'id' => 'search-form','class' => 'search_form']) !!} 
		                	<div class="col-xs-8 col-sm-6 col-md-4 pull-right">
		                  		<div class="input-group">
		                    		 <span id="search" class="input-group-addon"><i class="glyphicon glyphicon-search"></i></span>
		                     			...
		                     			<a href="{{ action('Admission\PostnatalDischargeController@index') }}" class="input-group-addon" id="search-reset"><i class="glyphicon glyphicon-remove"></i></a>
		                 		 </div>
		                	</div>
		                {!! Form::close() !!}
                        </div>
                ...
                <table class="table  table-bordered table-responsive"  id="data-list">
				    <thead>
				      <tr>
				        ...
				        <th class="sorting_by sorting_icon @if($order['sortby'] == 'DOB') sorting_{{$order['sortorder']}}_icon @endif" data-field="{{ SiteHelpers::encrypt_id('DOB') }}">Date of birth</th>
				        <th>DOA</th>
				     </tr>
				    </thead>
  				    <tbody>
    		       @if(count($results) > 0)
     				@for ($i = 0; $i <  @count($results); $i++)
				      <tr  class="{{ $results[$i]->discharge_status}}">
				        <td>{{ $i+1 }}</td>
				        <td>
				            <a href="{{ action('Admission\PostnatalDischargeController@postnatalDischargelist',SiteHelpers::encrypt_id($results[$i]->BabyId))}}">
				                {{  $results[$i]->BabyName }}
				            </a>
				        </td>
     				...
     				@endfor
			      @else
			        <tr><td colspan="5" class="text-center"><span>No Record Found</span></td></tr>
			      @endif 
    			  </tbody>
                </table>  
        <div class="row">
                  ...
                  <div class="dataTables_paginate paging_bootstrap pagination_footer">
                    <ul class="pagination">    
                      <li class="prev @if(Request::query('page') == '' || Request::query('page') == $pagination['start']) disabled @endif">
                        <a class="@if(Request::query('page') != $pagination['start'] &&  Request::query('page') != '') sort_with_page @endif" href="@if(Request::query('page') == $pagination['start'] || Request::query('page') == '') javascript:void(0); @else {{ url('postanatal-discharge?page='.$pagination['previous']) }}@endif">&#8592; Previous</a>
                      </li>
                    @for ($i = $pagination['start']; $i <= $pagination['end']; $i++) 
                      <li class="@if($i == Request::query('page')) active @elseif(Request::query('page') == '' && $i == $pagination['start']) active @endif">
                        <a class='sort_with_page' href="{{ url('postanatal-discharge?page='.$i) }}">{{$i}}</a>
                      </li>  
                    @endfor
                      <li class="next @if(Request::query('page') == $pagination['end'] || $pagination['total'] <= $pagination['limits']) disabled @endif">
                        <a class="@if(Request::query('page') != $pagination['end'] && $pagination['start'] != $pagination['end']) sort_with_page @endif" href="@if(Request::query('page') == $pagination['end'] || $pagination['total'] <= $pagination['limits']) javascript:void(0); @else {{ url('postanatal-discharge?page='.$pagination['next']) }}@endif">Next &#8594; </a>  
                      </li>
                    </ul>
                  </div>
                </div>
...
function DeleteData(id){
	bootbox.confirm("Are you sure?",function(confirmed){
		if(confirmed){
			$("#DeleteForm").attr('action',"{{ action('Admission\PostnatalDischargeController@index') }}/"+id);
			$("#DeleteForm").submit();
		}
	});
}
