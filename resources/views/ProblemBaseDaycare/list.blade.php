...
<div class="crumbs bread-crumbs-shadow">
	<ul id="breadcrumbs" class="breadcrumb">
		...
		<li class="current">
			<a href="{{ action('ProblemBaseDaycare\ProblemDaycareController@index') }}">NEONATAL PROBLEMS SECTION - NICU</a>
		</li>                                                
	</ul>
</div>
<!-- /Breadcrumbs line -->
				...
				<div id="data-list_wrapper" class="dataTables_wrapper form-inline" role="grid">
					<div class="row">
						<div class="dataTables_header clearfix">
							<div class="col-xs-4 col-md-6 col-sm-6">
								<div id="data-list_length" class="dataTables_length">
									<label class="data_limit">
										{!! Form::open(['url' => action('ProblemBaseDaycare\ProblemDaycareController@index'), 'method' => 'get', 'id' => 'limit-form']) !!}
							...
							{!! Form::open(['url' => action('ProblemBaseDaycare\ProblemDaycareController@index'), 'method' => 'get', 'id' => 'search-form','class' => 'search_form']) !!}
							<div class="col-xs-8 col-sm-6 col-md-4 pull-right">
								<div class="input-group">
									...
									<a href="{{ action('ProblemBaseDaycare\ProblemDaycareController@index') }}" class="input-group-addon" id="search-reset"><i class="glyphicon glyphicon-remove"></i></a>
								</div>
							</div>
							{!! Form::close() !!}
						</div>
				...
				<table class="table table-striped table-bordered table-responsive"  id="data-list">
					<thead>
						<tr>
							...
							<th class="sorting_by sorting_icon @if($order['sortby'] == 'BabyBloodGroup') sorting_{{$order['sortorder']}}_icon @endif hidden-xs" data-field="{{ SiteHelpers::encrypt_id('BabyBloodGroup') }}">Blood Group</th>
					...
					<tbody>
						...
						<tr>
							<td>
								...
								<a class="@if(!in_array('NICU_PROBLEM_DAY',$write_permission)) permission-denied @endif check-neonatal" href="@if(in_array('NICU_PROBLEM_DAY',$write_permission)) {{ action('ProblemBaseDaycare\ProblemDaycareController@admissionlist',\SiteHelpers::encrypt_id($results[$i]->BabyId) ) }} @else javascript:void(0); @endif" data-neonatal-id="{{$results[$i]->NeonatalId}}" data-baby-id="{{SiteHelpers::encrypt_id($results[$i]->BabyId)}}">
									{{  $results[$i]->BabyName }}
								</a>
								@else
								<a class="@if(!in_array('NICU_PROBLEM_DAY',$write_permission)) permission-denied @endif check-neonatal" data-neonatal-id="{{$results[$i]->NeonatalId}}" data-baby-id="{{SiteHelpers::encrypt_id($results[$i]->BabyId)}}">
									{{  $results[$i]->BabyName }}
								</a>
								@endif
							</td>
							<td>{{  $results[$i]->BMrNo }}</td>
							<td>@if(date('Y',strtotime($results[$i]->DOB)) > 1980) {{  date('d-m-Y',strtotime($results[$i]->DOB)) }} @endif</td>
			                  ...
			                  <div class="dataTables_paginate paging_bootstrap pagination_footer">
			                    <ul class="pagination">    
			                      <li class="prev @if(Request::query('page') == '' || Request::query('page') == $pagination['start']) disabled @endif">
			                        <a class="@if(Request::query('page') != $pagination['start'] &&  Request::query('page') != '') sort_with_page @endif" href="@if(Request::query('page') == $pagination['start'] || Request::query('page') == '') javascript:void(0); @else {{ url('problems-systems?page='.$pagination['previous']) }}@endif">&#8592; Previous</a>
			                      </li>
			                    @for ($i = $pagination['start']; $i <= $pagination['end']; $i++) 
			                      <li class="@if($i == Request::query('page')) active @elseif(Request::query('page') == '' && $i == $pagination['start']) active @endif">
			                        <a class='sort_with_page' href="{{ url('problems-systems?page='.$i) }}">{{$i}}</a>
			                      </li>  
			                    @endfor
			                      <li class="next @if(Request::query('page') == $pagination['end'] || $pagination['total'] <= $pagination['limits']) disabled @endif">
			                        <a class="@if(Request::query('page') != $pagination['end'] && $pagination['start'] != $pagination['end']) sort_with_page @endif" href="@if(Request::query('page') == $pagination['end'] || $pagination['total'] <= $pagination['limits']) javascript:void(0); @else {{ url('problems-systems?page='.$pagination['next']) }}@endif">Next &#8594; </a>  
			                      </li>
			                    </ul>
			                  </div>
			                </div>
