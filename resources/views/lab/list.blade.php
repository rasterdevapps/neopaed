...
<div class="crumbs bread-crumbs-shadow">
	<ul id="breadcrumbs" class="breadcrumb">
		...
		<li class="current">
			<a href="{{ action('Lab\LabRequestController@index') }}">Lab Request List</a>
		</li>                                                
	</ul>
</div>
<!--=== Page Content ===-->
<div class="row row-spacing">
	<div class="col-md-12">
       ...
       <h4>Lab Request List</h4>
       @if(in_array('LABREQUEST',$write_permission))
       <a href="{{ action('Lab\LabRequestController@create') }}" title="" class="btn btn-basic-shadow create-btn-spacing  btn-info pull-right create-btn">
     ...
     <div class="widget-content">
      <div id="data-list_wrapper" class="dataTables_wrapper form-inline" role="grid">
        <div class="row">
            ...
            <div class="col-xs-4 col-md-6">
              <div id="data-list_length" class="dataTables_length">
               <label class="data_limit">
                {!! Form::open(['url' => action('Lab\LabRequestController@index'), 'method' => 'get', 'id' => 'limit-form']) !!}
          ...
          {!! Form::open(['url' => action('Lab\LabRequestController@index'), 'method' => 'get', 'id' => 'search-form','class' => 'search_form']) !!} 
          <div class="col-xs-8 col-md-4 pull-right">
            <div class="input-group">
              ...
              <a href="{{ action('Lab\LabRequestController@index') }}" class="input-group-addon"><i class="glyphicon glyphicon-remove"></i></a>
            </div>                
          </div>
          {!! Form::close() !!}
        </div>
    ...
    <table class="table table-bordered table-responsive"  id="data-list">
      <thead>
        <tr >
          ...
          <th class="sorting_by sorting_icon @if($order['sortby'] == 'DOB') sorting_{{$order['sortorder']}}_icon @endif hidden-xs" data-field="{{ SiteHelpers::encrypt_id('DOB') }}" data-hide="tablet,phone">Date of birth</th>
          <th class="sorting_by sorting_icon @if($order['sortby'] == 'test_date') sorting_{{$order['sortorder']}}_icon @endif hidden-xs" data-field="{{ SiteHelpers::encrypt_id('test_date') }}" data-hide="tablet,phone">Date</th>
          @if(in_array('LABREQUEST',$write_permission))
          <th class="center-align-phone hide">Edit</th> 
          @endif   
          <th class="center-align-phone">Print</th>
          <th class="center-align-phone">Delete</th>
          <th class="center-align-phone hide">Print Result</th>
        </tr>
      </thead>
      <tbody>
        @if(count($results) > 0)  
        ...
        <tr class="">
         ...
         <td class="hidden-xs">{{  is_null($results[$i]->DOB) ? '':date('d-m-Y',strtotime($results[$i]->DOB)) }}</td>
         <td class="hidden-xs">{{  is_null($results[$i]->test_date) ?'' :date('d-m-Y', strtotime($results[$i]->test_date)) }}</td>
         @if(in_array('LABREQUEST',$write_permission))
         <td  class="center-align-phone hide">
           <a class="icon" href="{{ action('Lab\LabRequestController@edit', $results[$i]->id) }}">
         ...
         <td  class="center-align-phone">
          <a class="icon btn btn-warning btn-view" href="{{ action('Lab\LabRequestController@print', $results[$i]->id) }}">
       ...
       <td  class="center-align-phone hide">
        <a class="icon" href="{{ action('Lab\LabRequestController@print', $results[$i]->id) }}">
          <i class="fa fa-print"></i> 
          <span class="hidden-phone">Print Result</span>
        </a>
      </td>  
    </tr>
    @endfor
    @else
    <tr><td colspan="9" class="text-center"><span>No Record Found</span> </td></tr>
    @endif             
  </tbody>
</table>  
<div class="row">
        ...
        <div class="dataTables_paginate paging_bootstrap pagination_footer">
          <ul class="pagination">    
            <li class="prev @if(Request::query('page') == '' || Request::query('page') == $pagination['start']) disabled @endif">
              <a class="@if(Request::query('page') != $pagination['start'] &&  Request::query('page') != '') sort_with_page @endif" href="@if(Request::query('page') == $pagination['start'] || Request::query('page') == '') javascript:void(0); @else {{ url('daycare-admission?page='.$pagination['previous']) }}@endif">&#8592; Previous</a>
            </li>
            ...
            <li class="next @if(Request::query('page') == $pagination['end'] || $pagination['total'] <= $pagination['limits']) disabled @endif">
              <a class="@if(Request::query('page') != $pagination['end'] && $pagination['start'] != $pagination['end']) sort_with_page @endif" href="@if(Request::query('page') == $pagination['end'] || $pagination['total'] <= $pagination['limits']) javascript:void(0); @else {{ url('daycare-admission?page='.$pagination['next']) }}@endif">Next &#8594; </a>  
            </li>
          </ul>
        </div>
      </div>
...
<script type="text/javascript">
   ...
   bootbox.confirm("Are you sure?",function(confirmed){
    if(confirmed){
     $("#DeleteForm").attr('action',"{{ action('Lab\LabRequestController@index') }}/"+id);
     $("#DeleteForm").submit();
   }
 });
 }
