...
<div class="crumbs bread-crumbs-shadow">
  <ul id="breadcrumbs" class="breadcrumb ">
    ...
    <li class="current"><a href="{{ action('Masters\MediprobsController@index') }}">Medical Problems</a></li>
  </ul>
</div>
<!-- /Breadcrumbs line -->
<!--=== Page Content ===-->
  ...
  <div class="col-md-12">
    <div class="widget box table-view-shadow">
      <div class="widget-header">
        <h4>Medical Problems
        <i class="fa fa-info-circle bs-tooltip" data-placement="right" data-original-title="1.Neonatal Proforma -> Obstetric History -> Medical Problems. &nbsp; 2.NICU Admission -> Admission Proforma -> Medical History -> Medical Problems"></i></h4>
        @if(in_array('MAS_M_PROBLEM',$write_permission))
          <a href="{{ action('Masters\MediprobsController@create') }}" title="" class="btn btn-basic-shadow btn-info pull-right create-btn-spacing create-btn"><i class="fa fa-plus "></i> <span>Create New</span></a>
        @endif
      </div>
        ...
        <div id="data-list_wrapper" class="dataTables_wrapper form-inline" role="grid">
          <div class="row">
            <div class="dataTables_header clearfix" >
              <div class="col-xs-4 col-sm-6 col-md-6">
                <div id="data-list_length" class="dataTables_length">
                  <label class="data_limit">
                    {!! Form::open(['url' => action('Masters\MediprobsController@index'), 'method' => 'get', 'id' => 'limit-form']) !!}
              ...
              {!! Form::open(['url' => action('Masters\MediprobsController@index'), 'method' => 'get', 'id' => 'search-form','class' => 'search_form']) !!}
                <div class="col-xs-8 col-sm-6 col-md-4 pull-right">
                  <div class="input-group">
                    ...
                    <a href="{{ action('Masters\MediprobsController@index') }}" class="input-group-addon">
        ...
        <table class="table table-striped table-bordered table-responsive master-list">
          <thead>
            <tr>
              ...
              <th class="sorting_by sorting_icon @if($order['sortby'] == 'Name') sorting_{{$order['sortorder']}}_icon @endif" data-field="{{ SiteHelpers::encrypt_id('Name') }}">Medical Problem</th>
              ...
              @if(in_array('MAS_M_PROBLEM',$write_permission))
                <th class="center-align-phone">Edit</th>
                  @endif
              @if(in_array('MAS_M_PROBLEM',$delete_permission))
            ...
            @if(count($results) > 0 )
              @for ($i = 0; $i < @count($results); $i++)
                <tr>
                  ...
                  <a class="btn btn-info btn-view" href="{{ action('Masters\MediprobsController@edit',SiteHelpers::encrypt_id($results[$i]->Id)) }}" title="Edit Record">
                  ...
                  <div class="dataTables_paginate paging_bootstrap pagination_footer">
                    <ul class="pagination">
                      <li class="prev @if(Request::query('page') == '' || Request::query('page') == $pagination['start']) disabled @endif">
                        <a class="@if(Request::query('page') != $pagination['start'] &&  Request::query('page') != '') sort_with_page @endif" href="@if(Request::query('page') == $pagination['start'] || Request::query('page') == '') javascript:void(0); @else {{ url('masters/medicalproblems?page='.$pagination['previous']) }}@endif">&#8592; Previous</a>
                      </li>
                      @for ($i = $pagination['start']; $i <= $pagination['end']; $i++)
                        <li class="@if($i == Request::query('page')) active @elseif(Request::query('page') == '' && $i == $pagination['start']) active @endif">
                          <a class='sort_with_page' href="{{ url('masters/medicalproblems?page='.$i) }}">{{$i}}</a>
                        </li>
                      @endfor
                      <li class="next @if(Request::query('page') == $pagination['end'] || $pagination['total'] <= $pagination['limits']) disabled @endif">
                        <a class="@if(Request::query('page') != $pagination['end'] && $pagination['start'] != $pagination['end']) sort_with_page @endif" href="@if(Request::query('page') == $pagination['end'] || $pagination['total'] <= $pagination['limits']) javascript:void(0); @else {{ url('masters/medicalproblems?page='.$pagination['next']) }}@endif">Next &#8594; </a>
                      </li>
                    </ul>
                  </div>
                </div>
...
<script type="text/javascript">
  function DeleteData(id){
    bootbox.confirm("Are you sure?",function(confirmed){
      if(confirmed){
        $("#DeleteForm").attr('action',"{{ action('Masters\MediprobsController@index') }}/"+id);
        $("#DeleteForm").submit();
      }
    });
  }
