...
<?php $delete_permission = session('delete_permission'); ?>
	...
	<div class="crumbs bread-crumbs-shadow">
		<ul id="breadcrumbs" class="breadcrumb">
			...
			<li class="current"><a href="{{ action('Masters\MchatfollowupquestionsController@index') }}">M-CHAT-R Followup Questions</a></li>
		</ul>
	</div>
	<!-- /Breadcrumbs line -->
		...
		<div class="col-md-12">
			<div class="widget box table-view-shadow">
				<div class="widget-header">
					<h4>Investigation</h4>
					<a href="{{ action('Masters\MchatfollowupquestionsController@create') }}" class="btn btn-basic-shadow btn-info pull-right create-btn-spacing"><i class="fa fa-plus-circle"></i><span>Add (or) Format Questions</span></a>
				</div>
								...
								<tr>
					                ...
					                @if(in_array('MAS_M_CHAT_R_QUESTIONS',$write_permission))
						                <td class="center-align-phone">
						                  <a class="btn btn-info btn-view" href="{{ action('Masters\MchatfollowupquestionsController@edit',SiteHelpers::encrypt_id($results[$i]->id) ) }}" title="Edit Record">
...
<script type="text/javascript">
	function Delete(id) {
		bootbox.confirm("Are you sure?",function(confirmed) {
      		if(confirmed) {
              	$("#DeleteForm").attr('action',"{{ action('Masters\MchatfollowupquestionsController@index') }}/"+id);
        		$("#DeleteForm").submit();
      		}
    	});
	}
