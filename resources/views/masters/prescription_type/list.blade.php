...
<div class="crumbs bread-crumbs-shadow">
  <ul id="breadcrumbs" class="breadcrumb">
    ...
    <li class="current">
      <a href="{{ action('Masters\PrescriptionTypeController@index') }}">Prescription Type</a>
    </li>
  </ul>
</div>
<!-- /Breadcrumbs line -->
  ...
  <div class="col-md-12">
    <div class="widget box table-view-shadow">
      <div class="widget-header">
        ...
        @if(in_array('MAS_PRESCRIPTION_TYPE',$write_permission))
          <a href="{{ action('Masters\PrescriptionTypeController@create') }}" title="" class="btn btn-basic-shadow btn-info pull-right create-btn-spacing"><i class="fa fa-plus"></i> <span>Create New</span></a>
        @endif
      </div>
        ...
        <div id="data-list_wrapper" class="dataTables_wrapper form-inline" role="grid">
          <div class="row">
            <div class="dataTables_header clearfix" >
              <div class="col-xs-4 col-sm-6 col-md-6">
                <div id="data-list_length" class="dataTables_length">
                  <label class="data_limit">
                    {!! Form::open(['url' => action('Masters\PrescriptionTypeController@index'), 'method' => 'get', 'id' => 'limit-form']) !!}
              ...
              {!! Form::open(['url' => action('Masters\PrescriptionTypeController@index'), 'method' => 'get', 'id' => 'search-form','class' => 'search_form']) !!}
                <div class="col-xs-8 col-sm-6 col-md-4 pull-right">
                  <div class="input-group">
                    ...
                    <a href="{{ action('Masters\PrescriptionTypeController@index') }}" class="input-group-addon">
        ...
        <table class="table table-striped table-bordered table-responsive">
          <thead>
            <tr>
              ...
              @endif
              @if(in_array('MAS_PRESCRIPTION_TYPE',$delete_permission))
            ...
            @if(count($results) > 0 )
              @for ($i = 0; $i <  @count($results); $i++)
                <tr>
                  ...
                  <td>{{  $results[$i]->status == true ? 'Active' : 'Inactive' }}</td>
                  @if(in_array('MAS_PRESCRIPTION_TYPE',$write_permission))
                    <td class="center-align-phone">
                      <a class="btn btn-info btn-view" href="{{ action('Masters\PrescriptionTypeController@edit',SiteHelpers::encrypt_id($results[$i]->pres_id)) }}">
              ...
              @if(in_array('MAS_PRESCRIPTION_TYPE',$delete_permission))
                    <td class="center-align-phone">
                      <a class="icon btn btn-danger btn-remove" href="javascript:void(0);" onclick="DeleteData({{$results[$i]->pres_id}})">
                  ...
                  <div class="dataTables_paginate paging_bootstrap pagination_footer">
                    <ul class="pagination">
                      <li class="prev @if(Request::query('page') == '' || Request::query('page') == $pagination['start']) disabled @endif">
                        <a class="@if(Request::query('page') != $pagination['start'] &&  Request::query('page') != '') sort_with_page @endif" href="@if(Request::query('page') == $pagination['start'] || Request::query('page') == '') javascript:void(0); @else {{ url('masters/prescriptiontype?page='.$pagination['previous']) }}@endif">&#8592; Previous</a>
                      </li>
                      @for ($i = $pagination['start']; $i <= $pagination['end']; $i++)
                      <li class="@if($i == Request::query('page')) active @elseif(Request::query('page') == '' && $i == $pagination['start']) active @endif">
                        <a class='sort_with_page' href="{{ url('masters/prescriptiontype?page='.$i) }}">{{$i}}</a>
                      </li>
                      @endfor
                      <li class="next @if(Request::query('page') == $pagination['end'] || $pagination['total'] <= $pagination['limits']) disabled @endif">
                        <a class="@if(Request::query('page') != $pagination['end'] && $pagination['start'] != $pagination['end']) sort_with_page @endif" href="@if(Request::query('page') == $pagination['end'] || $pagination['total'] <= $pagination['limits']) javascript:void(0); @else {{ url('masters/prescriptiontype?page='.$pagination['next']) }}@endif">Next &#8594; </a>
                      </li>
                    </ul>
                  </div>
                </div>
...
<script type="text/javascript">
  function DeleteData(id){
    bootbox.confirm("Are you sure?",function(confirmed){
      if(confirmed){
        $("#DeleteForm").attr('action',"{{ action('Masters\PrescriptionTypeController@index') }}/"+id);
        $("#DeleteForm").submit();
      }
    });
  }
