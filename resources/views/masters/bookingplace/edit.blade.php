...
@section('content')
				...
				<div class="row row-spacing select-container-main">
					<div class="master-btn-layout">
                       {!! Form::model($results,['method'=> 'PATCH','url' => action('Masters\BookingPlaceController@update',$results->id),'id'=>'EditForm']) !!}
                       		 @include('masters.bookingplace.form',['SubmitButtonText'=>'Update'])
                       {!! Form::close() !!}
                      @include('errors.list')
					</div> <!-- /.col-md-12 -->
