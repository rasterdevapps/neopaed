    ...
    <div class="col-md-12">
        <div class="widget box table-view-shadow">
            <div class="widget-header">
                ...
                @if(in_array('MAS_INVESTIGATIONS',$write_permission))
                <a href="{{ action('Masters\InvestigationsController@create') }}" title="" class="btn btn-basic-shadow btn-info pull-right create-btn-spacing"><i class="fa fa-plus"></i> <span>Create New</span></a>
                @endif
            </div>
                        ...
                        <div class="dataTables_header clearfix" >
                            <div class="col-xs-4 col-sm-6 col-md-6">
                                <div id="data-list_length" class="dataTables_length">
                                    ...
                                    {!! Form::open(['url' => action('Masters\InvestigationsController@index'), 'method' => 'get', 'id' => 'limit-form']) !!}
                            ...
                            {!! Form::open(['url' => action('Masters\InvestigationsController@index'), 'method' => 'get', 'id' => 'search-form','class' => 'search_form']) !!}
                            <div class="col-xs-8 col-sm-6 col-md-4 pull-right">
                                <div class="input-group">
                                    ...
                                    <a href="{{ action('Masters\InvestigationsController@index') }}" class="input-group-addon">
                ...
                <table class="table table-striped table-bordered table-responsive">
                    <thead>
                        <tr>
                            ...
                            <th class="sorting_by sorting_icon @if($order['sortby'] == 'package_name') sorting_{{$order['sortorder']}}_icon @endif" data-field="{{ SiteHelpers::encrypt_id('package_name') }}" >Package Name</th>
                            <th class="sorting_by sorting_icon @if($order['sortby'] == 'test_name') sorting_{{$order['sortorder']}}_icon @endif" data-field="{{ SiteHelpers::encrypt_id('test_name') }}">Test Name</th>
                            <th class="sorting_by sorting_icon @if($order['sortby'] == 'package_status') sorting_{{$order['sortorder']}}_icon @endif" data-field="{{ SiteHelpers::encrypt_id('package_status') }}">Package Status</th>
                            @if(in_array('MAS_INVESTIGATIONS',$write_permission))
                            <th class="center-align-phone">Edit</th>
                            @endif
                            @if(in_array('MAS_INVESTIGATIONS',$delete_permission))
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>
                              @php $test_name = ValuelistHelpers::get_test_investigations_master($value->id); @endphp
                            ...
                            <td>{{  $value->package_status == 1 ? 'Active' : 'Inactive' }}</td>
                            @if(in_array('MAS_INVESTIGATIONS',$write_permission))
                            <td class="center-align-phone">
                                <a class="btn btn-info btn-view" href="{{ action('Masters\InvestigationsController@edit',SiteHelpers::encrypt_id($value->id)) }}">
                            ...
                            @if(in_array('MAS_INVESTIGATIONS',$delete_permission))
                            <td class="center-align-phone">
                                <a class="icon btn btn-danger btn-remove" href="javascript:void(0);" data-packagename="{{$value->id}}">
                                ...
                                <div class="dataTables_paginate paging_bootstrap pagination_footer">
                                    <ul class="pagination">
                                        <li class="prev @if(Request::query('page') == '' || Request::query('page') == $pagination['start']) disabled @endif">
                                            <a class="@if(Request::query('page') != $pagination['start'] &&  Request::query('page') != '') sort_with_page @endif" href="@if(Request::query('page') == $pagination['start'] || Request::query('page') == '') javascript:void(0); @else {{ url('masters/investigations?page='.$pagination['previous']) }}@endif">&#8592; Previous</a>
                                        </li>
                                        @for ($i = $pagination['start']; $i <= $pagination['end']; $i++)
                                        <li class="@if($i == Request::query('page')) active @elseif(Request::query('page') == '' && $i == $pagination['start']) active @endif">
                                            <a class='sort_with_page' href="{{ url('masters/investigations?page='.$i) }}">{{$i}}</a>
                                        </li>
                                        @endfor
                                        <li class="next @if(Request::query('page') == $pagination['end'] || $pagination['total'] <= $pagination['limits']) disabled @endif">
                                            <a class="@if(Request::query('page') != $pagination['end'] && $pagination['start'] != $pagination['end']) sort_with_page @endif" href="@if(Request::query('page') == $pagination['end'] || $pagination['total'] <= $pagination['limits']) javascript:void(0); @else {{ url('masters/investigations?page='.$pagination['next']) }}@endif">Next &#8594; </a>
                                        </li>
                                    </ul>
                                </div>
                            </div>
...
<script type="text/javascript">
    $('.btn-remove').on('click', function() {
      ...
      bootbox.confirm("Are you sure?",function(confirmed){
        if(confirmed){
          $("#DeleteForm").attr('action',"{{ action('Masters\InvestigationsController@index') }}/"+packagename);
          $("#DeleteForm").submit();
        }
      });
    })
