...
<div class="crumbs bread-crumbs-shadow">
    <ul id="breadcrumbs" class="breadcrumb">
        ...
        <li>
            <a href="{{ action('Ward\BabyWardController@index') }}">Monitor Data</a>
        </li>
        <li class="current">
            <a href="javascript:void();">{{$baby_details->BabyName.'-'.$baby_details->BMrNo}}</a>
        </li>
    </ul>
</div>
<!-- /Breadcrumbs line -->
                ...
                <div id="data-list_wrapper" class="dataTables_wrapper form-inline" role="grid">
                    <div class="row">
                        <div class="dataTables_header clearfix">
                            <div class="col-xs-4 col-sm-6 col-md-6">
                                <div id="data-list_length" class="dataTables_length">
                                    <label class="data_limit">
                                        {!! Form::open(['url' => action('fhirformate\MonitorFormatController@getMonitordetails',['babyId'=>SiteHelpers::encrypt_id($babyId), 'admissionId'=>SiteHelpers::encrypt_id($admissionId)]), 'method' => 'get', 'id' => 'limit-form']) !!}
                            ...
                            {!! Form::open(['url' => action('fhirformate\MonitorFormatController@getMonitordetails',['babyId'=>SiteHelpers::encrypt_id($babyId), 'admissionId'=>SiteHelpers::encrypt_id($admissionId)]), 'method' => 'get', 'id' => 'search-form','class' => 'search_form']) !!} 
                            <div class="col-xs-8 col-sm-6 col-md-4 pull-right">
                                <div class="input-group">
                                    ...
                                    <a href="{{ action('fhirformate\MonitorFormatController@getMonitordetails',['babyId'=>SiteHelpers::encrypt_id($babyId), 'admissionId'=>SiteHelpers::encrypt_id($admissionId)]) }}" class="input-group-addon"><i class="glyphicon glyphicon-remove"></i></a>
                                </div>
                            </div>
                            {!! Form::close() !!}
                        </div>
                ...
                <div class="overflow-auto">
                    <table class="table table-striped table-bordered table-hover table-checkable"  id="data-list">
                        <thead>
                            <tr>
                                <th>S.No</th>
                                <th class="sorting_by sorting_icon @if($order['sortby'] == 'result_date_time') sorting_{{$order['sortorder']}}_icon @endif" data-field="{{ SiteHelpers::encrypt_id('result_date_time') }}">Received Date & Time</th>
                        ...
                        <tbody>
                            @if(count($monitorObservation) != 0)
                            @php $i = 0; @endphp
                            @foreach($monitorObservation as $observation)
                            <tr>
                                <td>{{ $i + 1 }}</td>
