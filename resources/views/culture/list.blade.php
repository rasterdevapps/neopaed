	...
	<div class="col-md-12">
        <div class="widget box table-view-shadow">
         <div class="widget-header">
            <h4>Culture Registry</h4>
            @if(in_array('TEST_CULTURE',$write_permission))
            <a href="{{ action('Extras\CultureController@create') }}" title="" class="btn btn-info btn-basic-shadow pull-right create-btn-spacing create-btn"><i class="fa fa-plus"></i> <span>Create New</span></a>    
            @endif                            
        </div>
        <div class="widget-content">
            <div id="data-list_wrapper" class="dataTables_wrapper form-inline" role="grid">
                <div class="row">
                    <div class="dataTables_header clearfix" >
                        <div class="col-xs-4 col-sm-6 col-md-6">
                            <div id="data-list_length" class="dataTables_length">
                                <label class="data_limit">
                                    {!! Form::open(['url' => action('Extras\CultureController@index'), 'method' => 'get', 'id' => 'limit-form']) !!}
                        ...
                        {!! Form::open(['url' => action('Extras\CultureController@index'), 'method' => 'get', 'id' => 'search-form','class' => 'search_form']) !!} 
                        <div class="col-xs-8 col-sm-6 col-md-4 pull-right">
                            <div class="input-group">
                                ...
                                <a href="{{ action('Extras\CultureController@index') }}" class="input-group-addon">
            ...
            <table class="table table-striped table-bordered table-responsive">				
               <thead>
                <tr>
                    ...
                    <th class="sorting_by sorting_icon @if($order['sortby'] == 'BMrNo') sorting_{{$order['sortorder']}}_icon @endif hidden-xs" data-field="{{ SiteHelpers::encrypt_id('BMrNo') }}" data-hide="phone">{{ Lang::get('home.mrn') }}</th>
                    <th class="sorting_by sorting_icon @if($order['sortby'] == 'Specimen') sorting_{{$order['sortorder']}}_icon @endif hidden-xs" data-field="{{ SiteHelpers::encrypt_id('Specimen') }}" data-hide="phone">Specimen</th>
                    <th class="sorting_by sorting_icon @if($order['sortby'] == 'Isolate') sorting_{{$order['sortorder']}}_icon @endif hidden-xs" data-field="{{ SiteHelpers::encrypt_id('Isolate') }}" data-hide="phone">Isolate</th>                                
                    <th class="sorting_by sorting_icon @if($order['sortby'] == 'EntryDate') sorting_{{$order['sortorder']}}_icon @endif hidden-xs" data-field="{{ SiteHelpers::encrypt_id('EntryDate') }}">Date</th>
                    <th class="center-align-phone" data-hide="phone">Print</th>                                                                
            ...
            <tbody>
                ...
                <tr>
                    <td>{{ $i+1 }}</td>
                    <td>
                        <a class="@if(!in_array('TEST_CULTURE',$write_permission)) permission-denied @endif" href="@if(in_array('TEST_CULTURE',$write_permission)) {{ action('Extras\CultureController@edit', SiteHelpers::encrypt_id($results[$i]->CultureId)) }} @else javascript:void(0); @endif">
                            {{  $results[$i]->BabyName }}
                        </a>
                    </td>
                    ...
                    <td>{{  date('d-m-Y',strtotime($results[$i]->EntryDate)) }}</td>
                    @if(in_array('TEST_CULTURE',$read_permission))
                    <td class="center-align-phone">
                        <a class="btn btn-warning btn-view" href="{{ action('Extras\CultureController@printData',SiteHelpers::encrypt_id($results[$i]->CultureId)) }}">
                  ...
                  @if(in_array('TEST_CULTURE',$delete_permission))
                  <td class="center-align-phone">
                      <a class="btn btn-danger btn-view" href="javascript:void(0);" onclick="DeleteData({{ $results[$i]->CultureId }})">
                    ...
                    <div class="dataTables_paginate paging_bootstrap pagination_footer">
                        <ul class="pagination">    
                            <li class="prev @if(Request::query('page') == '' || Request::query('page') == $pagination['start']) disabled @endif">
                                <a class="@if(Request::query('page') != $pagination['start'] &&  Request::query('page') != '') sort_with_page @endif" href="@if(Request::query('page') == $pagination['start'] || Request::query('page') == '') javascript:void(0); @else {{ url('culture-registry?page='.$pagination['previous']) }}@endif">&#8592; Previous</a>
                            </li>
                            @for ($i = $pagination['start']; $i <= $pagination['end']; $i++) 
                            <li class="@if($i == Request::query('page')) active @elseif(Request::query('page') == '' && $i == $pagination['start']) active @endif">
                                <a class='sort_with_page' href="{{ url('culture-registry?page='.$i) }}">{{$i}}</a>
                            </li>  
                            @endfor
                            <li class="next @if(Request::query('page') == $pagination['end'] || $pagination['total'] <= $pagination['limits']) disabled @endif">
                                <a class="@if(Request::query('page') != $pagination['end'] && $pagination['start'] != $pagination['end']) sort_with_page @endif" href="@if(Request::query('page') == $pagination['end'] || $pagination['total'] <= $pagination['limits']) javascript:void(0); @else {{ url('culture-registry?page='.$pagination['next']) }}@endif">Next &#8594; </a>  
                            </li>
                        </ul>
                    </div>
                </div>
...
<script type="text/javascript">
    function DeleteData(id){
       bootbox.confirm("Are you sure?",function(confirmed){
          if(confirmed){
             $("#DeleteForm").attr('action',"{{ action('Extras\CultureController@index') }}/"+id);
             $("#DeleteForm").submit();
         }
     });
   }
