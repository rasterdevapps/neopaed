...
<div class="crumbs bread-crumbs-shadow">
    ...
    <ul class="pull-right nicu-list discharge-color-code1 list-none hide">
        <li>
            <a class='btn btn-info btn-basic-shadow search-btn' href="{{ action('Search\PediatricController@create').'?module=admission' }}">
    ...
    <div class="col-md-12">
        <div class="widget box table-view-shadow">
            <div class="widget-header">
                ...
                <a href="{{ action('Admission\PediatricController@show',SiteHelpers::encrypt_id(0)).'/create' }}" title="" class="btn btn-info btn-basic-shadow pull-right create-btn-spacing"><i class="fa fa-plus"></i><span>Create New</span></a>  
                @endif                              
            </div>
                ...
                <div id="data-list_wrapper" class="dataTables_wrapper form-inline" role="grid">
                    <div class="row">
                        <div class="dataTables_header clearfix">
                            <div class="col-xs-4 col-sm-6 col-md-6">
                                <div id="data-list_length" class="dataTables_length">
                                    <label class="data_limit">
                                        {!! Form::open(['url' => action('Admission\PediatricController@index'), 'method' => 'get', 'id' => 'limit-form']) !!}
                            ...
                            {!! Form::open(['url' => action('Admission\PediatricController@index'), 'method' => 'get', 'id' => 'search-form','class' => 'search_form']) !!}
                            <div class="col-xs-8 col-sm-6 col-md-4 pull-right">
                                <div class="input-group">
                                    ...
                                    <a href="{{ action('Admission\PediatricController@index') }}" class="input-group-addon">
                ...
                <table class="table table-striped table-bordered table-responsive" id="data-list">
                    <thead>
                        <tr>
                            ...
                            <th class="sorting_by sorting_icon @if($order['sortby'] == 'BMrNo') sorting_{{$order['sortorder']}}_icon @endif hidden-xs" data-field="{{ SiteHelpers::encrypt_id('BMrNo') }}">{{ Lang::get('home.mrn') }}</th>
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td>
                                <a class="icon" href="{{ action('Admission\PediatricController@sublist',\SiteHelpers::encrypt_id($results[$i]->BabyId)) }}">
                                    {{  $results[$i]->BabyName }}
                                </a>
                            </td>
                            ...
                            <td>
                                ...
                                @if (isset($admission_date_list[$results[$i]->BMrNo]))    
                                @foreach ($admission_date_list[$results[$i]->BMrNo] as $key => $admission)
                                    @php 
                                ...
                                <div class="dataTables_paginate paging_bootstrap pagination_footer">
                                    <ul class="pagination">
                                        <li class="prev @if(Request::query('page') == '' || Request::query('page') == $pagination['start']) disabled @endif">
                                            <a class="@if(Request::query('page') != $pagination['start'] &&  Request::query('page') != '') sort_with_page @endif" href="@if(Request::query('page') == $pagination['start'] || Request::query('page') == '') javascript:void(0); @else {{ url('pediatric-admission?page='.$pagination['previous']) }}@endif">&#8592; Previous</a>
                                        </li>
                                        @for ($i = $pagination['start']; $i <= $pagination['end']; $i++) 
                                        <li class="@if($i == Request::query('page')) active @elseif(Request::query('page') == '' && $i == $pagination['start']) active @endif">
                                            <a class='sort_with_page' href="{{ url('pediatric-admission?page='.$i) }}">{{$i}}</a>
                                        </li>
                                        @endfor
                                        <li class="next @if(Request::query('page') == $pagination['end'] || $pagination['total'] <= $pagination['limits']) disabled @endif">
                                            <a class="@if(Request::query('page') != $pagination['end'] && $pagination['start'] != $pagination['end']) sort_with_page @endif" href="@if(Request::query('page') == $pagination['end'] || $pagination['total'] <= $pagination['limits']) javascript:void(0); @else {{ url('pediatric-admission?page='.$pagination['next']) }}@endif">Next &#8594; </a>  
                                        </li>
                                    </ul>
                                </div>
                            </div>
