...
<div class="crumbs bread-crumbs-shadow">
    ...
    <ul class="pull-right nicu-list discharge-color-code1 list-none hidden-xs hidden-sm">
        <li>
            <a class='btn btn-info btn-basic-shadow search-btn' href="{{ action('Search\PostnatalController@create').'?module=admission' }}">
	...
	<div class="col-md-12">
        <div class="widget box table-view-shadow">
			<div class="widget-header">
			    <h4>Postnatal Registered Patients</h4>
                @include('admission_filter')
			    @if(in_array('MOTHER_REG',$write_permission))
					   <a href="{{ action('Admission\PostnatalController@create') }}" title="create new" class="btn btn-basic-shadow btn-info pull-right create-btn-spacing create-btn">
       			...
       			<div id="data-list_wrapper" class="dataTables_wrapper form-inline" role="grid">
                    <div class="row">
			            <div class="dataTables_header clearfix" >
			                <div class="col-xs-4 col-sm-6 col-md-6">
				                <div id="data-list_length" class="dataTables_length ">
				                    <label class="data_limit">
				                      {!! Form::open(['url' => action('Admission\PostnatalController@index'), 'method' => 'get', 'id' => 'limit-form']) !!}
		                 ...
		                 {!! Form::open(['url' => action('Admission\PostnatalController@index'), 'method' => 'get', 'id' => 'search-form','class' => 'search_form']) !!} 
		                	<div class="col-xs-8 col-sm-6 col-md-4 pull-right">
		                  		<div class="input-group">
		                    		 <span id="search" class="input-group-addon"><i class="glyphicon glyphicon-search"></i></span>
		                     			...
		                     			<a href="{{ action('Admission\PostnatalController@index') }}" class="input-group-addon" id="search-reset"><i class="glyphicon glyphicon-remove"></i></a>
		                 		 </div>
		                	</div>
		                {!! Form::close() !!}
                        </div>
                ...
                <table class="table  table-bordered table-responsive"  id="data-list">
				    <thead>
				      <tr>
				        ...
				        <th class="sorting_by sorting_icon @if($order['sortby'] == 'BMrNo') sorting_{{$order['sortorder']}}_icon @endif hidden-xs" data-field="{{ SiteHelpers::encrypt_id('BMrNo') }}" data-hide="phone,tablet">{{ Lang::get('home.mrn') }}</th>
    		       ...
    		       @if(count($results) > 0)
     				@for ($i = 0; $i <  @count($results); $i++)
				      <tr>
				        <td>{{ $i+1 }}</td>
				        <td>
				            <a href="{{ action('Admission\PostnatalController@postnatalAdmissionlist',SiteHelpers::encrypt_id($results[$i]->BabyId))}}">
				                {{  $results[$i]->BabyName }}
				            </a>
				        </td>
		                  ...
		                  <div class="dataTables_paginate paging_bootstrap pagination_footer">
		                    <ul class="pagination">    
		                      <li class="prev @if(Request::query('page') == '' || Request::query('page') == $pagination['start']) disabled @endif">
		                        <a class="@if(Request::query('page') != $pagination['start'] &&  Request::query('page') != '') sort_with_page @endif" href="@if(Request::query('page') == $pagination['start'] || Request::query('page') == '') javascript:void(0); @else {{ url('postanatal-admission?page='.$pagination['previous']) }}@endif">&#8592; Previous</a>
		                      </li>
		                    @for ($i = $pagination['start']; $i <= $pagination['end']; $i++) 
		                      <li class="@if($i == Request::query('page')) active @elseif(Request::query('page') == '' && $i == $pagination['start']) active @endif">
		                        <a class='sort_with_page' href="{{ url('postanatal-admission?page='.$i) }}">{{$i}}</a>
		                      </li>  
		                    @endfor
		                      <li class="next @if(Request::query('page') == $pagination['end'] || $pagination['total'] <= $pagination['limits']) disabled @endif">
		                        <a class="@if(Request::query('page') != $pagination['end'] && $pagination['start'] != $pagination['end']) sort_with_page @endif" href="@if(Request::query('page') == $pagination['end'] || $pagination['total'] <= $pagination['limits']) javascript:void(0); @else {{ url('postanatal-admission?page='.$pagination['next']) }}@endif">Next &#8594; </a>  
		                      </li>
		                    </ul>
		                  </div>
		                </div>
