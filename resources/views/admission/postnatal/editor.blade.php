...
<head>
    ...
    <title>Postnatal Admission Report</title>
    <style type="text/css">
        .print-tool-bar {
            float: right;
        }
...
<body>
    ...
    <div class="container">
        {!! Form::model(null,['method' => 'POST','url' => action('Admission\PostnatalController@saveFullEditor',null),'id'=>'post-reports']) !!}
        {!! Form::hidden('post_id',$dischage_summary['post_id']) !!}
        <textarea name="post_report" id="post_report">
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            ...
            CKEDITOR.instances["post_report"].on('blur',function() {
                postReportSave();
            });
            ...
            function postReportSave($slug=true) {
                   var report=CKEDITOR.instances['post_report'].getData();
                   $('textarea[name="post_report"]').val(report);
                  ...
                  type    :"POST",
                  url     : "{{ url('postnatal-reports-save') }}",
                  data    :$('#post-reports').serialize(),
                  cache   : false,
                  dataType: "json",
                  success:function(response) {
                            if ($slug) {
                                ...
                                window.location = "{{ url('postanatal-admission') }}";                        
                            }
                },
