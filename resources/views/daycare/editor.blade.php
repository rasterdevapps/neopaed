...
<head>
    ...
    <title>Nicu Daycare Report</title>
    <style type="text/css">
        .print-tool-bar {
            float: right;
        }
...
<body>
    <div class="print-tool-bar">
        ...
        <a href="#" id="daycare-close" class="btn" title="Back">
            <i class="fa fa-times fa-2x" aria-hidden="true"></i>
        </a>
    </div>
    <div class="container">
        {!! Form::model(null,['method' => 'POST','url' => action('Admission\DaycareController@saveFullEditor',null),'id'=>'daycare-reports']) !!}
        {!! Form::hidden('day_id',$dischage_summary['day_id']) !!}
        <textarea name="nicu_daycare_report" id="nicu_daycare_report">
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            CKEDITOR.replace('nicu_daycare_report', {
                toolbar: [{
                    name: 'document',
                    items: ['Print']
                },
                ...
                contentsCss: ["{{ $site_url}}/css/bootstrap.min.css", "{{ $site_url}}/css/print.css", "{{ $site_url}}/css/customized.css"],
                allowedContent: true,
            });

            CKEDITOR.instances["nicu_daycare_report"].on('blur',function() {
                reportSave();                
            });
            $('.save-report').click(function() {
                $('#daycare-reports').submit();
            });
            ...
            $('#daycare-close').click(function() {
                reportSave(false);  
            });

            function reportSave($slug=true) {
                    var report=CKEDITOR.instances['nicu_daycare_report'].getData();
                    $('textarea[name="nicu_daycare_report"]').val(report);
                    ...
                    $.ajax({
                        ...
                        type    :"POST",
                        url     : "{{ url('nicu-daycare-reports-save') }}",
                        data    :$('#daycare-reports').serialize(),
                        cache   : false,
                        dataType: "json",
                        success:function(response) {
                            if ($slug) {
                                ...
                                window.location = "{{ url('daycare-admission') }}";                        
                            }
                        },
