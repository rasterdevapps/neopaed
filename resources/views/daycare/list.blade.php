...
{{ Session::put('daycare-create-slug','1') }}
<!-- Breadcrumbs line -->
<div class="crumbs bread-crumbs-shadow">
	<ul id="breadcrumbs" class="breadcrumb">
		<li><i class="icon-home"></i><a href="{{url('/')}}">{{ Lang::get('home.daycare_dashboard') }}</a></li>
		<li class="current"><a href="{{ action('Admission\DaycareController@index') }}">{{ Lang::get('home.daycare_name') }}</a></li>                                                
	</ul>
  <ul class="pull-right list-none daycare-search">
    <li><a href="{{ action('Search\SearchDaycareController@create') }}" class="btn btn-info btn-basic-shadow mr-15 search-btn">
      <i class="fa fa-search"></i>  
    <span>{{ Lang::get('home.daycare_search') }}</span>
  </a>
  </li>
  </ul>
</div>
	...
	<div class="col-md-12">
		<div class="widget box table-view-shadow">
			<div class="widget-header">
				<h4>{{ Lang::get('home.daycare_name') }}</h4>
                @include('admission_filter')
        @if(in_array('NICU_DAY',$write_permission))
					<a href="{{ action('Admission\DaycareController@create') }}" title="" class="btn btn-basic-shadow create-btn-spacing  btn-info pull-right create-btn"><i class="fa fa-plus "></i> <span>{{ Lang::get('home.daycare_create_new')}}</span></a>  
        @endif                              
			</div>
       ...
       <div id="data-list_wrapper" class="dataTables_wrapper form-inline" role="grid">
            ...
            <div class="col-xs-4 col-md-6 col-sm-6">
              <div id="data-list_length" class="dataTables_length">
                <label class="data_limit">
                  {!! Form::open(['url' => action('Admission\DaycareController@index'), 'method' => 'get', 'id' => 'limit-form']) !!}
                        ...
                        </select><span class="hidden-xs">{{ Lang::get('home.daycare_records')}}</span>
        ...
        {!! Form::open(['url' => action('Admission\DaycareController@index'), 'method' => 'get', 'id' => 'search-form','class' => 'search_form']) !!} 
                ...
                <div class="input-group">
                     <span id="search" class="input-group-addon"><i class="glyphicon glyphicon-search"></i></span>
                      <input type="text" aria-controls="data-list" class="form-control" name="search_txt" value="{{ $search['search_txt'] }}" placeholder="{{ Lang::get('home.daycare_search_placeholder') }}">
                     <a href="{{ action('Admission\DaycareController@index') }}" class="input-group-addon" id="search-reset"><i class="glyphicon glyphicon-remove"></i></a>
                  </div>                
                </div>
       {!! Form::close() !!}
               </div>
        ...
        <thead>
            <tr >
                <th>{{ Lang::get('home.daycare_sno')}}</th>
                <th class="sorting_by sorting_icon @if($order['sortby'] == 'BabyName') sorting_{{$order['sortorder']}}_icon @endif" data-field="{{ SiteHelpers::encrypt_id('BabyName') }}">{{ Lang::get('home.daycare_baby_name')}}</th>
                ...
                <th class="sorting_by sorting_icon @if($order['sortby'] == 'DOB') sorting_{{$order['sortorder']}}_icon @endif" data-field="{{ SiteHelpers::encrypt_id('DOB') }}" data-hide="tablet,phone">{{ Lang::get('home.daycare_dob')}}</th>
             @if($hide=='true')
               <th class="sorting_by sorting_icon @if($order['sortby'] == 'DayDate') sorting_{{$order['sortorder']}}_icon @endif" data-field="{{ SiteHelpers::encrypt_id('DayDate') }}" data-hide="tablet,phone">{{ Lang::get('home.daycare_date')}}</th>
                @if(in_array('NICU_DAY',$write_permission))
                <th >{{ Lang::get('home.daycare_edit')}}</th>  
                <th >{{ Lang::get('home.daycare_delete')}}</th>
                @endif   
                <th>{{ Lang::get('home.daycare_preview')}}</th>
                <th>{{ Lang::get('home.daycare_Print')}}</th>   
             @endif                                                                                
            </tr>
        </thead>
        <tbody>
      @if(count($results) > 0)  
        @for ($i = 0; $i <  @count($results); $i++)
          <tr class="{{ $results[$i]->rowcolor }}">
             ...
             <a href="{{ url('daycare-admission/daycare-baby-admissionlist/'.\SiteHelpers::encrypt_id($results[$i]->BabyId)) }}" class="check-neonatal" data-neonatal-id="{{$results[$i]->NeonatalId}}" data-baby-id="{{SiteHelpers::encrypt_id($results[$i]->BabyId)}}">
              {{  $results[$i]->BabyName }} 
              </a>
              @else
             <a class="check-neonatal" data-neonatal-id="{{$results[$i]->NeonatalId}}" data-baby-id="{{SiteHelpers::encrypt_id($results[$i]->BabyId)}}">
              ...
              <td  class="center-align-phone">
                 <a class="icon" onclick="ShowModal({!!  $results[$i]->DayId !!},'{{ action('Admission\DaycareController@getData',$results[$i]->DayId) }}')" href="javascript:void(0);">
                   <i class="fa fa-eye"></i> 
                   <span class="hidden-phone">{{ Lang::get('home.daycare_view')}}</span>
                 </a>
              </td>          
              <td  class="center-align-phone"><a class="icon" href="{{ action('Admission\DaycareController@printData',$results[$i]->DayId) }}">
              <i class="fa fa-print"></i> 
              <span class="hidden-phone">{{ Lang::get('home.daycare_Print')}}</span>
              </a>
              </td> 
              @endif                               
          </tr>
          ...
          <div class="col-md-12">
            <div class="dataTables_footer clearfix">
              <div class="col-md-6 col-sm-6 col-xs-12">
                ...
                {{ Lang::get('home.daycare_showing')}} {{$pagination['limit'][0]}} {{ Lang::get('home.daycare_to') }} {{$pagination['limit'][1]}} {{ Lang::get('home.daycare_of') }} {{$pagination['total']}} {{ Lang::get('home.daycare_entries') }} {{ $search['search_txt'] != '' ? '(filtered from '. $getTotal .' total entries)' : '' }}
                </div>
              </div>
                <div class="col-md-6 col-sm-6 col-xs-12">
                  <div class="dataTables_paginate paging_bootstrap pagination_footer">
                    <ul class="pagination">    
                      <li class="prev @if(Request::query('page') == '' || Request::query('page') == $pagination['start']) disabled @endif">
                        <a class="@if(Request::query('page') != $pagination['start'] &&  Request::query('page') != '') sort_with_page @endif" href="@if(Request::query('page') == $pagination['start'] || Request::query('page') == '') javascript:void(0); @else {{ url('daycare-admission?page='.$pagination['previous']) }}@endif">&#8592; {{ Lang::get('home.daycare_previous')}}</a>
                      </li>
                    @for ($i = $pagination['start']; $i <= $pagination['end']; $i++) 
                      <li class="@if($i == Request::query('page')) active @elseif(Request::query('page') == '' && $i == $pagination['start']) active @endif">
                        <a class='sort_with_page' href="{{ url('daycare-admission?page='.$i) }}">{{$i}}</a>
                      </li>  
                    @endfor
                      <li class="next @if(Request::query('page') == $pagination['end'] || $pagination['total'] <= $pagination['limits']) disabled @endif">
                        <a class="@if(Request::query('page') != $pagination['end'] && $pagination['start'] != $pagination['end']) sort_with_page @endif" href="@if(Request::query('page') == $pagination['end'] || $pagination['total'] <= $pagination['limits']) javascript:void(0); @else {{ url('daycare-admission?page='.$pagination['next']) }}@endif">{{ Lang::get('home.daycare_next')}} &#8594; </a>  
                      </li>
                    </ul>
                  </div>
                </div>
