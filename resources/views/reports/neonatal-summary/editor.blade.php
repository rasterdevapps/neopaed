...
<head>
    ...
    <title>Postnatal Discharge Summary</title>
    <style type="text/css">
        .print-tool-bar {
            float: right;
        }
...
<body>
    <div class="print-tool-bar">
        ...
        <a href="#" id="post-disch-close" class="btn" title="Back">
            <i class="fa fa-times fa-2x" aria-hidden="true"></i>
        </a>
    </div>
    <div class="container">
        {!! Form::model(null,['method' => 'POST','url' => action('Reports\PostnatalDischargeSummary@saveFullEditor',null),'id'=>'post-summary-reports']) !!}
        {!! Form::hidden('baby_id',$dischage_summary['baby_id']) !!}
        {!! Form::hidden('admission_id',$dischage_summary['admission_id']) !!}
        <textarea name="post_summary" id="post_summary">
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            ...
            CKEDITOR.instances["post_summary"].on('blur',function() {
                postDischargeReportSave();
            });
            $('.save-report').click(function() {
                $('#post-summary-reports').submit();
            });
            $(document).on('click', '#cke_16', function() {
                postDischargeReportSave();
            });
            $('#post-disch-close').click(function() {
                postDischargeReportSave(false);  
            });
            function postDischargeReportSave($slug=true) {
                var report=CKEDITOR.instances['post_summary'].getData();
                $('textarea[name="post_summary"]').val(report);
                ...
                $.ajax({
                  ...
                  type    :"POST",
                  url     : "{{ url('postnatal-summary-reports-save') }}",
                  data    :$('#post-summary-reports').serialize(),
                  cache   : false,
                  dataType: "json",
                  success:function(response) {
                    if ($slug) {
                        ...
                        window.location = "{{ url('postproblem-systems-summary') }}";                        
                    }
                },
