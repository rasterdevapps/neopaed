...
<?php $delete_permission = session('delete_permission'); ?>
 ...
 <div class="crumbs bread-crumbs-shadow">
   <ul id="breadcrumbs" class="breadcrumb">
    ...
    <li class="current"><a href="{{ action('ProblemsSettings\ProblemsSettingController@index') }}">Daycare Problems</a></li>                                                
  </ul>
</div>
	...
	<div class="col-md-12">
		<div class="widget box table-view-shadow">
			<div class="widget-header">
				<h4>Daycare Problems <i class="fa fa-info-circle bs-tooltip" data-placement="right" data-original-title=""></i></h4>
				@if(in_array('MAS_PROBLEM_SETTING',$write_permission))
       <a href="{{ action('ProblemsSettings\ProblemsSettingController@create') }}" title="Create New Problem" class="btn btn-info btn-basic-shadow pull-right create-btn-spacing"><i class="fa fa-plus "></i> <span>Create New</span></a>      
       @endif                          
     </div>
      ...
      <div id="data-list_wrapper" class="dataTables_wrapper form-inline" role="grid">
        <div class="row">
          <div class="dataTables_header clearfix">
            <div class="col-xs-4 col-sm-6 col-md-6">
              <div id="data-list_length" class="dataTables_length">
                <label class="data_limit">
                  {!! Form::open(['url' => action('ProblemsSettings\ProblemsSettingController@index'), 'method' => 'get', 'id' => 'limit-form']) !!}
            ...
            {!! Form::open(['url' => action('ProblemsSettings\ProblemsSettingController@index'), 'method' => 'get', 'id' => 'search-form','class' => 'search_form']) !!} 
            <div class="col-xs-8 col-sm-6 col-md-4 pull-right">
              <div class="input-group">
               ...
               <a href="{{ action('ProblemsSettings\ProblemsSettingController@index') }}" class="input-group-addon"><i class="glyphicon glyphicon-remove"></i></a>
             </div>                
           </div>
           {!! Form::close() !!}
         </div>
     ...
     <table class="table table-striped table-bordered table-responsive "  id="data-list">
      <thead>
        <tr>
          ...
          <th class="sorting_by sorting_icon @if($order['sortby'] == 'problem_name') sorting_{{$order['sortorder']}}_icon @endif" data-field="{{ SiteHelpers::encrypt_id('problem_name') }}">Daycare Problems</th>
          <th class="sorting_by sorting_icon @if($order['sortby'] == 'problem_status') sorting_{{$order['sortorder']}}_icon @endif" data-field="{{ SiteHelpers::encrypt_id('problem_status') }}">Status</th>
          @if(in_array('MAS_PROBLEM_SETTING',$delete_permission))
          <th class="center-align-phone">Delete</th>
          @endif
        </tr>
      </thead>
      <tbody>
        @if(count($problemslist) > 0 )
        @for ($i = 0; $i <  @count($problemslist); $i++)
        <tr>
          <td class="hidden-xs">{{ $i+1 }}</td>
          <td>
            <a href="@if(in_array('MAS_PROBLEM_SETTING',$write_permission)) {{ action('ProblemsSettings\ProblemsSettingController@edit',SiteHelpers::encrypt_id($problemslist[$i]->problem_id)) }} @else javascript:void(); @endif">
             {{  $problemslist[$i]->problem_name }}
           </a>
         </td>
         <td>{{  ($problemslist[$i]->problem_status == 1) ? 'Active' : 'Inactive' }}</td>
         @if(in_array('MAS_PROBLEM_SETTING',$delete_permission))
         <td class="center-align-phone">
          <a class="icon btn btn-danger btn-remove" href="javascript:" onclick="DeleteData({{$problemslist[$i]->problem_id}})" title="Delete Record">
          ...
          <div class="dataTables_paginate paging_bootstrap pagination_footer">
            <ul class="pagination">    
              <li class="prev @if(Request::query('page') == '' || Request::query('page') == $pagination['start']) disabled @endif">
                <a class="@if(Request::query('page') != $pagination['start'] &&  Request::query('page') != '') sort_with_page @endif" href="@if(Request::query('page') == $pagination['start'] || Request::query('page') == '') javascript:void(0); @else {{ url('problems-settings?page='.$pagination['previous']) }}@endif">&#8592; Previous</a>
              </li>
              @for ($i = $pagination['start']; $i <= $pagination['end']; $i++) 
              <li class="@if($i == Request::query('page')) active @elseif(Request::query('page') == '' && $i == $pagination['start']) active @endif">
                <a class='sort_with_page' href="{{ url('problems-settings?page='.$i) }}">{{$i}}</a>
              </li>  
              @endfor
              <li class="next @if(Request::query('page') == $pagination['end'] || $pagination['total'] <= $pagination['limits']) disabled @endif">
                <a class="@if(Request::query('page') != $pagination['end'] && $pagination['start'] != $pagination['end']) sort_with_page @endif" href="@if(Request::query('page') == $pagination['end'] || $pagination['total'] <= $pagination['limits']) javascript:void(0); @else {{ url('problems-settings?page='.$pagination['next']) }}@endif">Next &#8594; </a>  
              </li>
            </ul>
          </div>
        </div>
...
<script type="text/javascript">
  function DeleteData(id){
    bootbox.confirm("Are you sure?",function(confirmed){
     if(confirmed){
      $("#DeleteForm").attr('action',"{{ action('ProblemsSettings\ProblemsSettingController@index') }}/"+id);
      $("#DeleteForm").submit();
    }
  });
  }
