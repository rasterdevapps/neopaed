...
<div class="crumbs bread-crumbs-shadow">
	<ul id="breadcrumbs" class="breadcrumb">
		...
		<li class="current"><a href="{{ action('Extras\UltraController@index') }}">Cranial Ultrasonography</a></li>                                                
	</ul>
</div>
<!-- /Breadcrumbs line -->
<!--=== Page Content ===-->
<div class="row row-spacing">
    ...
    <div class="widget box table-view-shadow">
     <div class="widget-header">
      <h4>Cranial Ultrasonography</h4>
      @if(in_array('TEST_ULTRA',$write_permission))
      <a href="{{ action('Extras\UltraController@create') }}" title="" class="btn btn-basic-shadow btn-info pull-right create-btn-spacing create-btn"><i class="fa fa-plus "></i> <span>Create New</span></a>                                
      @endif
    </div>
    <div class="widget-content">     
      <div id="data-list_wrapper" class="dataTables_wrapper form-inline" role="grid">
        <div class="row">
           ...
           <div class="col-xs-4 col-sm-6 col-md-6">
            <div id="data-list_length" class="dataTables_length">
              <label class="data_limit">
                {!! Form::open(['url' => action('Extras\UltraController@index'), 'method' => 'get', 'id' => 'limit-form']) !!}
          ...
          {!! Form::open(['url' => action('Extras\UltraController@index'), 'method' => 'get', 'id' => 'search-form','class' => 'search_form']) !!}
          <div class="col-xs-8 col-sm-6 col-md-4 pull-right"> 
            <div class="input-group">
             ...
             <a href="{{ action('Extras\UltraController@index') }}" class="input-group-addon"><i class="glyphicon glyphicon-remove"></i></a>
           </div>   
         </div>
         {!! Form::close() !!}
       </div>
     </div>
     <table class="table table-striped table-bordered table-responsive dataTable mt-0"  id="data-list">				
      <thead>
        <tr>
         <th>S.No.</th>
         ...
         <th class="sorting_by sorting_icon @if($order['sortby'] == 'DOB') sorting_{{$order['sortorder']}}_icon @endif hidden-phone" data-field="{{ SiteHelpers::encrypt_id('DOB') }}">DOB</th>
         <th class="sorting_by sorting_icon @if($order['sortby'] == 'TestDate') sorting_{{$order['sortorder']}}_icon @endif hidden-phone" data-field="{{ SiteHelpers::encrypt_id('TestDate') }}">Date</th>
         <th class="hidden-xs" data-hide="phone">Print</th>                                
                           @if(in_array('TEST_ULTRA',$delete_permission))
                       ...
                       <tbody>
                        ...
                        <tr>
                          <td>{{  $i+1 }}</td>
                          <td>
                            <a class="icon @if(!in_array('TEST_ULTRA',$write_permission)) permission-denied @endif" href="@if(in_array('TEST_ULTRA',$write_permission)) {{ action('Extras\UltraController@edit', SiteHelpers::encrypt_id($results[$i]->UltraId)) }} @else javascript:void(0); @endif">
                              {{  $results[$i]->BabyName }}
                            </a>
                          </td>
                          <td>{{  $results[$i]->BMrNo }}</td>
                          <td class="hidden-phone">@if(date('Y',strtotime( $results[$i]->DOB)) > 1970)  {{  date('d-m-Y',strtotime($results[$i]->DOB)) }} @endif </td>
                          <td class="hidden-phone">{{  date('d-m-Y',strtotime($results[$i]->TestDate)) }}</td>
                          <td class="hidden-phone">
                            <a class="btn btn-warning btn-view" href="{{ action('Extras\UltraController@printData', SiteHelpers::encrypt_id($results[$i]->UltraId)) }}">
                              <i class="fa fa-print"></i> 
                            </a>
                          </td>                                  
                            @if(in_array('TEST_ULTRA',$delete_permission))
                          <td class="hidden-phone">
                            <a class="btn btn-danger btn-view" href="javascript:void(0);" onclick="DeleteData({{ $results[$i]->UltraId }})">
                        ...
                        @endfor
                        @else
                        <tr><td colspan="7" class="text-center"><span>No Record Found</span> </td></tr>
                        @endif   
                      </tbody>
                    </table>
                    <div class="row">
                        ...
                        <div class="dataTables_footer clearfix">
                          <div class="col-md-6">
                           <div class="dataTables_info  pagination_info" id="DataTables_Table_0_info">
                             Showing {{$pagination['limit'][0]}} to {{count($results)}} of {{$pagination['total']}} entries {{ @$search_txt != '' ? '(filtered from '. $getTotal .' total entries)' : '' }}
                           </div>
                         </div>
                         <div class="col-md-6">
                          @php $condition = ($search_txt !='') ? '&search_txt='.$search_txt : '' ; @endphp
                          <div class="dataTables_paginate paging_bootstrap pagination_footer">
                            <ul class="pagination">    
                              <li class="prev @if(Request::query('page') == '' || Request::query('page') == $pagination['start']) disabled @endif">
                                <a class="@if(Request::query('page') != $pagination['start'] &&  Request::query('page') != '') sort_with_page @endif" href="@if(Request::query('page') == $pagination['start'] || Request::query('page') == '') javascript:void(0); @else {{url('cranialultrasound?page='.$pagination['previous'].$condition)}}@endif">&#8592; Previous</a>
                              </li>
                              @for ($i = $pagination['start']; $i <= $pagination['end']; $i++) 
                              <li class="@if($i == Request::query('page')) active @elseif(Request::query('page') == '' && $i == $pagination['start']) active @endif">
                                <a class='sort_with_page' href="{{url('cranialultrasound?page='.$i.$condition)}}">{{$i}}</a>
                              </li>  
                              @endfor
                              <li class="next @if(Request::query('page') == $pagination['end'] || $pagination['total'] <= $pagination['limits']) disabled @endif">
                                <a class="@if(Request::query('page') != $pagination['end'] && $pagination['start'] != $pagination['end']) sort_with_page @endif" href="@if(Request::query('page') == $pagination['end'] || $pagination['total'] <= $pagination['limits']) javascript:void(0); @else {{url('cranialultrasound?page='.$pagination['next'].$condition)}}@endif">Next &#8594; </a>  
                              </li>
                            </ul>
                          </div>
                        </div>
...
<script type="text/javascript">
    ...
    $.ajax({
     ...
     success :function(response){
      ...
      EditLink = '';
      @if(in_array('TEST_ULTRA',$write_permission))
      EditLink = '<a href="/cranialultrasound/'+id+'/edit/" class=""><i class="fa fa-pencil"></i></a>';
      @endif
      PrintLink = '<a href="/cranialultrasound/'+id+'/printdata/" class=""><i class="fa fa-print"></i></a>';								  			  $(".modal-title").html(Datas['BabyName']+' '+EditLink+ ' '+ PrintLink);				  
      ...
      $(".col-indication").html(Datas['Indication']);				  				  				  				
      $(".col-usg-general").html(Datas['UsgGeneral']);				  				  				  				
      $(".col-usg-rt").html(Datas['UsgRt']);				  				  				  				 						
      $(".col-usg-lt").html(Datas['UsgLt']);				  				  				  				
    },
    complete: function(){
  ...
  function DeleteData(id){
    bootbox.confirm("Are you sure?",function(confirmed){
     if(confirmed){
      $("#DeleteForm").attr('action',"{{ action('Extras\UltraController@index') }}/"+id);
      $("#DeleteForm").submit();
    }
  });
  }
