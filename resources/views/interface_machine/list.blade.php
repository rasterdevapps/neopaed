			...
			<div class="widget-header">
      		...
      		<div class="widget-content">
        		<div id="data-list_wrapper" class="dataTables_wrapper form-inline" role="grid">
          			<div class="row">
           				<div class="dataTables_header clearfix">
			              	<div class="col-xs-4 col-sm-6 col-md-6">
				                <div id="data-list_length" class="dataTables_length">
				                  <label class="data_limit">
				                      	{!! Form::open(['url' => action('interfacelog\InterfaceMachineController@index'), 'method' => 'get', 'id' => 'limit-form']) !!}
				            ...
				            {!! Form::open(['url' => action('interfacelog\InterfaceMachineController@index'), 'method' => 'get', 'id' => 'search-form','class' => 'search_form']) !!}
				            <div class="col-xs-8 col-sm-6 col-md-4 pull-right"> 
				                 <div class="input-group">
				                    <span id="search" class="input-group-addon">
				                      <i class="glyphicon glyphicon-search"></i>
				                    </span>
				                    <input type="text" aria-controls="data-list" class="form-control" name="search_txt" value="{{ $search_txt or '' }}" placeholder="Search">
				                    <a href="{{ action('interfacelog\InterfaceMachineController@index') }}" class="input-group-addon">
		    	...
		    	<table class="table table-striped table-bordered table-responsive" id="data-list">
		    	  	<thead>
		            	<tr>
				            <th class="sorting_by sorting_icon @if($order['sortby'] == 'received_ip') sorting_{{$order['sortorder']}}_icon @endif" data-field="{{ SiteHelpers::encrypt_id('received_ip') }}">Received Ip</th>
				            <th class="sorting_by sorting_icon @if($order['sortby'] == 'received_time') sorting_{{$order['sortorder']}}_icon @endif" data-field="{{ SiteHelpers::encrypt_id('received_time') }}">Received Time</th>
				            <th class="sorting_by sorting_icon @if($order['sortby'] == 'received_date') sorting_{{$order['sortorder']}}_icon @endif" data-field="{{ SiteHelpers::encrypt_id('received_date') }}">Received Date</th>
				            <th class="sorting_by sorting_icon @if($order['sortby'] == 'is_parsed') sorting_{{$order['sortorder']}}_icon @endif" data-field="{{ SiteHelpers::encrypt_id('is_parsed') }}">Is Parsed</th>
			            </tr>
			        </thead>
		          	<tbody>
		          	@if(count($lists) > 0)
		                ...
		                @foreach($lists as $list)  
		                    <tr class="detailinfo">
		                    	<td><a href="{{ action('interfacelog\InterfaceMachineController@show',$lists[$i]->id) }}">{{ $lists[$i]->received_ip }}</a></td>                    
		                    	<td>{{ date('h:i a', strtotime($lists[$i]->received_time)) }}</td>                    
		                    	<td>{{ date( 'd-m-Y',strtotime($lists[$i]->received_date)) }}</td> 
		                    	<td>{{ ($lists[$i]->is_parsed == 1) ? 'Yes' : 'No' }}</td>                    
		                  ...
		                  <div class="dataTables_paginate paging_bootstrap pagination_footer">
		                    <ul class="pagination">    
		                      <li class="prev @if(Request::query('page') == '' || Request::query('page') == $pagination['start']) disabled @endif">
		                        <a class="@if(Request::query('page') != $pagination['start'] &&  Request::query('page') != '') sort_with_page @endif" href="@if(Request::query('page') == $pagination['start'] || Request::query('page') == '') javascript:void(0); @else {{ url('interface-log?page='.$pagination['previous']) }}@endif">&#8592; Previous</a>
		                      </li>
		                    @for ($i = $pagination['start']; $i <= $pagination['end']; $i++) 
		                      <li class="@if($i == Request::query('page')) active @elseif(Request::query('page') == '' && $i == $pagination['start']) active @endif">
		                        <a class='sort_with_page' href="{{ url('interface-log?page='.$i) }}">{{$i}}</a>
		                      </li>  
		                    @endfor
		                      <li class="next @if(Request::query('page') == $pagination['end'] || $pagination['total'] <= $pagination['limits']) disabled @endif">
		                        <a class="@if(Request::query('page') != $pagination['end'] && $pagination['start'] != $pagination['end']) sort_with_page @endif" href="@if(Request::query('page') == $pagination['end'] || $pagination['total'] <= $pagination['limits']) javascript:void(0); @else {{ url('interface-log?page='.$pagination['next']) }}@endif">Next &#8594; </a>  
		                      </li>
		                    </ul>
		                  </div>
		                </div>
