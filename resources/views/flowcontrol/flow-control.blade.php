    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            ...
            <div class="modal-body">
                {!! Form::open(['url'=>action('Flow\FlowController@flowControl'), 'method'=>'GET','id'=>'flow-control']) !!}
                    ...
                    <div class="tab-content">
                        <div class="tab-pane" id="tab2">
                            <div class="col-md-12 center-window">
                                ...
                                {!! Form::hidden('registration',null,['id'=>'registration']) !!}
                            </div>
                        </div>
                        <div class="tab-pane" id="tab3">
                            <div class="col-md-12 center-window">
                                <div>
                                    @if(in_array('NICU_FORM', \Session::get('write_permission')))
                                    ...
                                    @endif
                                    @if(in_array('POST_FORM', \Session::get('write_permission')))
                                    ...
                                    @endif
                                    @if(in_array('PEDI_FORM', \Session::get('write_permission')))
                                ...
                                <div>
                                    @if(in_array('NEONATAL_OP_BASIC', \Session::get('write_permission')))
                                    @if(in_array('OP_REG', \Session::get('write_permission')))
                                    ...
                                    @endif
                                    @endif
                                    @if(in_array('PEDIATRICS_OP_REG', \Session::get('write_permission')))
                                    ...
                                    @endif
                                    @if(in_array('NEURO_DEVELOPMENT', \Session::get('write_permission')))
                                ...
                                @if(!in_array('NICU_FORM', \Session::get('write_permission')) && !in_array('POST_FORM', \Session::get('write_permission')) && !in_array('OP_REG', \Session::get('write_permission')) && !in_array('PEDI_FORM', \Session::get('write_permission')) && !in_array('PEDIATRICS_OP_REG', \Session::get('write_permission')) && !in_array('NEURO_DEVELOPMENT', \Session::get('write_permission')))
                                <h1>Permission Denied</h1>
                                @endif
                                {!! Form::hidden('admission',null,['id'=>'admission']) !!}
                            </div>
                            <!-- <div class="col-md-12">
...
<div class="modal fade flow-control-modal" id="daily-care-nav" role="dialog">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            ...
            <div class="modal-body">
                {!! Form::open(['url'=>action('Flow\FlowController@flowControl'), 'method'=>'GET','id'=>'flow-control-dailycare']) !!}
                {!! Form::hidden('registration',null,['id'=>'daily-care-registration']) !!}
                {!! Form::hidden('admission',null,['id'=>'daily-care-admission']) !!}
                <div class="daily-care-option">
                    <h3>Do You Want Add The Daily Sheet In ?</h3>
                    <div class="modal-divider">
                        @if(in_array('NICU_DAY',\Session::get('write_permission')))
                        <button  type="button" value="NICU_DAILY_CARE" class="btnn btn-primary daily-care-sheet daily-option btn-shadow-special"> 
                        NICU 
                        </button>
                        @endif
                        @if(in_array('POST_DAY',\Session::get('write_permission')))
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <div class="daily-care-option">
                    ...
                    <div class="modal-divider">
                        @if(in_array('NICU_PROBLEM_DAY',\Session::get('write_permission')))
                        <a href="{{ url('daycare-admission/create/problem-based') }}" class="btnn btn-primary daily-option btn-shadow-special lh-120"> 
                        NICU </a>
                        @endif 
                        @if(in_array('POST_PROBLEM_SYSTEM',\Session::get('write_permission')))
                        <a href="{{ url('problem-systems-postnatal') }}" class="btnn btn-primary daily-option btn-shadow-special lh-120"> 
                        Postnatal</a>
                        @endif 
                    </div>
                </div>
...
<div class="hidden">
    {!! Form::select('nicu_discharge_baby_id',[''=>'N/A']+SiteHelpers::getNicuBabyies(),null) !!}
    {!! Form::select('postanatal_discharge_baby_id',[''=>'N/A']+SiteHelpers::getPostnatalBabyies(),null) !!}
</div>
<div class="modal flow-control-modal fade" id="discharge-details-nav" role="dialog">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            ...
            <div class="modal-body">
                {!! Form::hidden('baby_id',null) !!}
                {!! Form::hidden('admission_id',null) !!}
                {!! Form::hidden('ward_id',null) !!}
                {!! Form::hidden('ward_name',null) !!}
                {!! Form::hidden('room_id',null) !!}
                {!! Form::hidden('room_name',null) !!}
                {!! Form::hidden('bed_id',null) !!}
                {!! Form::hidden('bed_name',null) !!}
                {!! Form::hidden('bed_mrn',null) !!}
                {!! Form::hidden('bed_ip',null) !!}
                <div id="discharge-flow">
                    ...
                    <div class="tab-content">
                        ...
                        <div class="tab-pane" id="discharge-step2">
                            ...
                            {!! Form::open(['url'=>action('Flow\FlowController@flowControl'), 'method'=>'GET','id'=>'flow-control-discharge', 'style' => 'margin:0px;']) !!}
                                {!! Form::hidden('registration',null,['id'=>'discharge-registration']) !!}
                                {!! Form::hidden('admission',null,['id'=>'discharge-admission']) !!}
                                    <div class="col-md-6">
                                        <div class="form-group">
                                            {!! Form::label('baby_id','Baby\'s Name:') !!}
                                            {!! Form::select('baby_id',['0'=>'N/A']+SiteHelpers::getNicuBabyies(),null,['class'=>'full-width-must', 'id'=>'discharge-baby-nicu']) !!}
                                        </div>
                                    </div>
                                    <div class="col-md-6">
                                        <div class="form-group">
                                            {!! Form::label('admission_id','Admission:') !!}
                                            {!! Form::select('admission_id',[''=>'N/A'],null,['class'=>'form-control','id'=>'admission_id']) !!}

                                        </div>
                                        <div class="form-group row">
                                            <div class="nicu_bed_details col-md-7" style="display:none;">
                                                {!! Form::label('nicu_bed','NICU Bed No:') !!}
                            ...
                            <div class="col-md-12" id="transfer-option">
                                {!! Form::open(['url'=>action('Ward\BabyWardController@getupdatebedlog'), 'method'=>'POST','id'=>'bed-transfer']) !!}
                                    ...
                                    {!! Form::hidden('bed_mrno',null) !!}
                                    {!! Form::hidden('bed_ipnumber',null) !!}
                                    {!! Form::hidden('tbed_name',null) !!}
                                    {!! Form::hidden('hms_ward_id',null) !!}
                                    {!! Form::hidden('hms_room_id',null) !!}
                                    {!! Form::hidden('hms_bed_id',null) !!}
            ...
            <div class="modal-body">
                <div class="col-md-12">
                    <div class="col-md-6">
                        <div class="form-group">
                            {!! Form::label('baby_id','Baby\'s Name:') !!}
                            {!! Form::select('baby_id',[''=>'N/A']+SiteHelpers::getNicuBabyies(),null,['class'=>'form-control', 'id'=>'nurse-sheet-baby']) !!}
                        </div>
                    </div>
                    <div class="col-md-6">
                        <div class="form-group">
                            {!! Form::label('admission_id','Admission:') !!}
                            {!! Form::select('admission_id',[''=>'N/A'],null,['class'=>'form-control','id'=>'nurse-sheet-admission-id']) !!}
                        </div>
                        <div class="form-group">
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <div class="row mtb-50">
                    {!! Form::open(['url'=>action('Flow\FlowController@nurseFlowcontrol'), 'method'=>'GET','id'=>'nurse-registration']) !!}
                    <div class="col-md-12 center-window">
                        ...
                        {!! Form::hidden('nurse_registration',null,['id'=>'nurse_registration']) !!}
                        {!! Form::hidden('nurse_admission',null,['id'=>'nurse_admission']) !!}
                    </div>
                    <!--  <div class="col-md-12">
            ...
            <div class="modal-body row mt-20">
                ...
                <div class="record col-md-12">
                    <div class="form-group col-md-12">
                        {{ Form::label('baby_name','Baby\'s Name:') }}
                        @php $baby_list = isset($active_baby_list) ? $active_baby_list : ['N/A'=>'N/A']; @endphp
                        {{ Form::select('baby_name',[0=>'- - Select - -']+$baby_list, null, ['class'=>'select2-select-00 input-fields-shadow full-width-fix']) }}
                    </div>
...
<script type="text/javascript">
    $(document).ready(function() {
       ...
       function nicuNursesheet(babyId) {
            ...
            $.ajax({
                    type:'GET',
                    url:'{{ url("nicu-admission-discharge") }}'+'/'+babyId,
                    beforeSend:function() {
        ...
        function postnatalDischarge(babyId) {
            ...
            $.ajax({
                    type:'GET',
                    url:'{{ url("postanatal-admission-discharge") }}'+'/'+babyId,
                    beforeSend:function() {
