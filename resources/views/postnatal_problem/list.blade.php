...
<div class="crumbs bread-crumbs-shadow">
	<ul id="breadcrumbs" class="breadcrumb">
		...
		<li class="current">
			<a href="{{ action('ProblemBaseDaycare\ProblemPostnatalController@index') }}">NEONATAL PROBLEMS SECTION - POSTNATAL </a>
		</li>                                                
	</ul>
</div>
<!-- /Breadcrumbs line -->
			    ...
			    <div id="data-list_wrapper" class="dataTables_wrapper form-inline" role="grid">
		            <div class="row">
			            <div class="dataTables_header clearfix" >
			                <div class="col-xs-4 col-sm-6 col-md-6">
			                    <div id="data-list_length" class="dataTables_length">
				                    <label class="data_limit">
				                     {!! Form::open(['url' => action('ProblemBaseDaycare\ProblemPostnatalController@index'), 'method' => 'get', 'id' => 'limit-form']) !!}
			                ...
			                {!! Form::open(['url' => action('ProblemBaseDaycare\ProblemPostnatalController@index'), 'method' => 'get', 'id' => 'search-form','class' => 'search_form']) !!} 
			                ...
			                <div class="input-group">
			                     ...
			                     <a href="{{ action('ProblemBaseDaycare\ProblemPostnatalController@index') }}" class="input-group-addon" id="search-reset"><i class="glyphicon glyphicon-remove"></i></a>
			                  </div>                
			                </div>
			                {!! Form::close() !!}
			            </div>
			    ...
			    <table class="table table-striped table-bordered table-responsive"  id="data-list">
	                <thead>
	                    <tr>
	                       ...
	                       <th class="sorting_by sorting_icon @if($order['sortby'] == 'BabyBloodGroup') sorting_{{$order['sortorder']}}_icon @endif" data-field="{{ SiteHelpers::encrypt_id('BabyBloodGroup') }}">Blood Group</th>
	                ...
	                @if(count($results) > 0)  	
		                @for ($i = 0; $i <  @count($results); $i++)
		                    <tr>
		                        <td>
		                        	<a class="@if(!in_array('POST_PROBLEM_SYSTEM',$write_permission)) permission-denied @endif" href="@if(in_array('POST_PROBLEM_SYSTEM',$write_permission)) {{ action('ProblemBaseDaycare\ProblemPostnatalController@admissionlist',SiteHelpers::encrypt_id($results[$i]->BabyId)) }} @else javascript:void(0); @endif">
		                        	{{  $results[$i]->BabyName }}
		                        	</a>
		                        </td>
		                        <td>{{  $results[$i]->BMrNo }}</td>
		                        <td>@if(date('Y',strtotime($results[$i]->DOB)) > 1980) {{  date('d-m-Y',strtotime($results[$i]->DOB)) }} @endif</td>                                            
		                        <td>{{  $results[$i]->BabyBloodGroup }}</td>
		                    </tr>
		                @endfor
		            @else
		                  ...
		                  <div class="dataTables_paginate paging_bootstrap pagination_footer">
		                    <ul class="pagination">    
		                      <li class="prev @if(Request::query('page') == '' || Request::query('page') == $pagination['start']) disabled @endif">
		                        <a class="@if(Request::query('page') != $pagination['start'] &&  Request::query('page') != '') sort_with_page @endif" href="@if(Request::query('page') == $pagination['start'] || Request::query('page') == '') javascript:void(0); @else {{ url('problem-systems-postnatal?page='.$pagination['previous']) }}@endif">&#8592; Previous</a>
		                      </li>
		                    @for ($i = $pagination['start']; $i <= $pagination['end']; $i++) 
		                      <li class="@if($i == Request::query('page')) active @elseif(Request::query('page') == '' && $i == $pagination['start']) active @endif">
		                        <a class='sort_with_page' href="{{ url('problem-systems-postnatal?page='.$i) }}">{{$i}}</a>
		                      </li>  
		                    @endfor
		                      <li class="next @if(Request::query('page') == $pagination['end'] || $pagination['total'] <= $pagination['limits']) disabled @endif">
		                        <a class="@if(Request::query('page') != $pagination['end'] && $pagination['start'] != $pagination['end']) sort_with_page @endif" href="@if(Request::query('page') == $pagination['end'] || $pagination['total'] <= $pagination['limits']) javascript:void(0); @else {{ url('problem-systems-postnatal?page='.$pagination['next']) }}@endif">Next &#8594; </a>  
		                      </li>
		                    </ul>
		                  </div>
		                </div>
